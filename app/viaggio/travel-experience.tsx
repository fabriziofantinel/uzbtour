...
import {
  ...
  Sparkles, Star, TrainFront, Trash2, Trophy, Utensils, Wallet,
} from "lucide-react";
...
type Tab = "oggi" | "mappa" | "programma" | "ricordi" | "spese" | "info" | "frasario" | "sfide" | "classifica";
...
export default function TravelExperience({ initialExperience, userName, isAgencyAdmin = false }: {
  ...
  async function addCash(kind: "withdrawal" | "exchange") {
    ...
    const euroRaw = prompt(kind === "withdrawal" ? "Importo addebitato in euro?" : "Importo cambiato in euro?");
  ...
  return <main>
    ...
    <nav className="tabs"><button className={tab === "mappa" ? "active" : ""} onClick={() => setTab("mappa")}><Map/> Mappa</button><button className={tab === "programma" ? "active" : ""} onClick={openProgramme}><CalendarDays/> Programma</button><button className={tab === "spese" ? "active" : ""} onClick={() => setTab("spese")}><Wallet/> Spese, prelievi, cambi</button><button className={tab === "info" ? "active" : ""} onClick={() => setTab("info")}><Info/> Info utili</button><button className={tab === "frasario" ? "active" : ""} onClick={() => setTab("frasario")}><Languages/> Frasi</button><button className={tab === "sfide" ? "active" : ""} onClick={() => setTab("sfide")}><Sparkles/> Sfide</button><button className={tab === "classifica" ? "active" : ""} onClick={() => setTab("classifica")}><Trophy/> Classifica</button></nav>
    ...
    {tab === "classifica" && <PlatformTripRankings experience={experience} userName={userName}/>}
