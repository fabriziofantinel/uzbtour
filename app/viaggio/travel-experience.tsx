...
export default function TravelExperience({ initialExperience, userName, isAgencyAdmin = false }: {
  ...
  return <main>
    ...
    <nav className="tabs"><button className={tab === "mappa" ? "active" : ""} onClick={() => setTab("mappa")}><Map/> Mappa</button><button className={tab === "programma" ? "active" : ""} onClick={openProgramme}><CalendarDays/> Programma</button><button className={tab === "spese" ? "active" : ""} onClick={() => setTab("spese")}><Wallet/> Spese, prelievi, cambi <b>€ {totals.EUR.toFixed(2)} · {som.format(totals.UZS)} UZS</b></button><button className={tab === "info" ? "active" : ""} onClick={() => setTab("info")}><Info/> Info utili</button><button className={tab === "frasario" ? "active" : ""} onClick={() => setTab("frasario")}><Languages/> Frasi</button><button className={tab === "sfide" ? "active" : ""} onClick={() => setTab("sfide")}><Sparkles/> Sfide</button></nav>
