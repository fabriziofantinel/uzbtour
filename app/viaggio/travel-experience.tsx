...
function mapCityForDay(day: Day) {
  ...
  const mentionedCities = cities
    .map((city) => ({ city, position: routeText.lastIndexOf(normalizedLocation(city.name)) }))
...
export default function TravelExperience({
  ...
  useEffect(() => {
    setLargeText(localStorage.getItem("smf-large-text") === "1");
    setSimpleMode(localStorage.getItem("smf-simple-mode") === "1");
  }, []);
  ...
  function togglePreference(kind: "text" | "simple") {
    if (kind === "text")
      setLargeText((value) => {
        localStorage.setItem("smf-large-text", value ? "0" : "1");
        return !value;
      });
    else
      setSimpleMode((value) => {
        localStorage.setItem("smf-simple-mode", value ? "0" : "1");
        return !value;
      });
  }
  if (!day) return null;
  ...
  return (
    ...
    >
            ...
            {operationalPoints.length > 0 ? (
              <>
                <div className="overviewMap">
                  <TripOverviewMap days={operationalPoints} onSelect={() => undefined} />
                </div>
                ...
                {displayedUsefulInfo.map((item, index) => (
                  <article key={`${item.title}-${index}`}>
                    ...
                    <footer className="usefulGovernance">
                      ...
                      {item.verifiedAt && (
                        <time>Verificato il {new Intl.DateTimeFormat("it-IT").format(new Date(item.verifiedAt))}</time>
                      )}
                      {item.sourceUrl && (
          ...
          <section className="collection sosPage">
            <header>
              ...
              <div>
                ...
                <p>
                  Consulta le istruzioni e contatta subito i servizi competenti. SMF Travel non condivide né conserva la
                  tua posizione.
                </p>
              </div>
            </header>
      ...
      <CashMovementDialog
        ...
        onSave={(input) => (cashDialogKind ? addCash(cashDialogKind, input) : Promise.resolve(false))}
      />
    </main>
  );
}
