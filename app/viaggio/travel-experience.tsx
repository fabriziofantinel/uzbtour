...
const usefulSections = [
  { key: "ambasciata", title: "Ambasciata" }, { key: "salute", title: "Salute" }, { key: "documenti", title: "Documenti" },
...
export default function TravelExperience({ initialExperience, userName, isAgencyAdmin = false }: {
  ...
  const emergencyInfo = useMemo(() => experience.usefulInfo.find((item) => `${item.category} ${item.title}`.toLocaleLowerCase("it").includes("numeri di emergenza")), [experience.usefulInfo]);
  ...
  const operationalPoints = useMemo<TripMapDay[]>(() => {
    const points = experience.days.flatMap((entry, dayIndex) => [
      ...entry.cities.filter((city) => city.latitude != null && city.longitude != null).map((city) => ({ title: city.name, city: city.name, lat: city.latitude!, lon: city.longitude! })),
      ...entry.items.filter((item) => item.latitude != null && item.longitude != null).map((item) => ({ title: item.title, city: entry.city || item.title, lat: item.latitude!, lon: item.longitude! })),
      ...entry.sites.filter((site) => site.latitude != null && site.longitude != null).map((site) => ({ title: site.name, city: site.city || entry.city, lat: site.latitude!, lon: site.longitude! })),
      ...entry.hotels.filter((hotel) => hotel.latitude != null && hotel.longitude != null).map((hotel) => ({ title: hotel.name, city: hotel.city || entry.city, lat: hotel.latitude!, lon: hotel.longitude! })),
    ].map((point) => ({ ...point, dayIndex, dayNumber: entry.number, date: dateParts(entry.date).full })));
    return [...new globalThis.Map(points.map((point) => [`${point.dayNumber}:${point.lat}:${point.lon}:${point.title}`, point] as const)).values()]
      .map((point, index) => ({ index, n: point.dayNumber, date: point.date, color: colors[point.dayIndex % colors.length], title: point.title, city: point.city, lat: point.lat, lon: point.lon }));
  }, [experience.days]);
  ...
  useEffect(() => {
    const restoreTab = () => {
      ...
      if (["programma", "documenti", "spese", "mappa", "sfide", "info", "frasario", "sos", "chat"].includes(requestedTab || "")) {
  ...
  function openDay(index: number) { selectDay(index); selectTab("programma"); }
  ...
  return <main className={`travelExperience travelerRedesign${largeText?" largeTextMode":""}${simpleMode?" simpleMode":""}`} style={brandStyle} data-design-contract="b0beb44b" data-design-thesis="sentiero-delle-tappe"
    ...
    <nav className="tabs" aria-label="Sezioni del viaggio"><button type="button" className={tab === "mappa" ? "active" : ""} aria-current={tab === "mappa" ? "page" : undefined} onClick={() => { selectTab("mappa"); setMoreOpen(false); }}><Map/><span>Mappa</span></button><button type="button" className={tab === "programma" ? "active" : ""} aria-current={tab === "programma" ? "page" : undefined} onClick={() => { openProgramme(); setMoreOpen(false); }}><CalendarDays/><span>Programma</span></button><button type="button" className={tab === "documenti" ? "active" : ""} aria-current={tab === "documenti" ? "page" : undefined} onClick={() => { selectTab("documenti"); setMoreOpen(false); }}><FileText/><span>Documenti</span></button><button type="button" aria-label="Spese, prelievi e cambi" className={tab === "spese" ? "active" : ""} aria-current={tab === "spese" ? "page" : undefined} onClick={() => { selectTab("spese"); setMoreOpen(false); }}><Wallet/><span>Spese</span></button><button type="button" className={tab === "sfide" ? "active" : ""} aria-current={tab === "sfide" ? "page" : undefined} onClick={() => { selectTab("sfide"); setMoreOpen(false); }}><Sparkles/><span>Sfide</span></button><button ref={moreButtonRef} type="button" className={moreOpen || ["ricordi", "info", "frasario"].includes(tab) ? "active" : ""} aria-expanded={moreOpen} aria-haspopup="true" aria-controls="travel-more-menu" onClick={() => setMoreOpen((value) => !value)}><CircleUserRound/><span>Altro</span></button></nav>
    {moreOpen && <div ref={moreMenuRef} id="travel-more-menu" className="moreMenu" role="region" aria-label="Altre sezioni">
      ...
      <button type="button" aria-current={tab === "info" ? "page" : undefined} onClick={() => { selectTab("info"); setMoreOpen(false); }}><Info/><span><strong>Informazioni utili</strong><small>Contatti, valuta e consigli</small></span><ChevronRight/></button>
      <button type="button" aria-current={tab === "frasario" ? "page" : undefined} onClick={() => { selectTab("frasario"); setMoreOpen(false); }}><Languages/><span><strong>Frasi</strong><small>Parole utili durante il viaggio</small></span><ChevronRight/></button>
      <button type="button" aria-current={tab === "chat" ? "page" : undefined} onClick={() => { selectTab("chat"); setMoreOpen(false); }}><MessageCircle/><span><strong>Chat con l’agenzia</strong><small>Domande e aggiornamenti operativi</small></span><ChevronRight/></button>
      <button type="button" aria-current={tab === "sos" ? "page" : undefined} onClick={() => { selectTab("sos"); setMoreOpen(false); }}><ShieldAlert/><span><strong>SOS e assistenza</strong><small>Contatti e posizione volontaria</small></span><ChevronRight/></button>
    ...
    {tab === "mappa" && <section className="overviewPage"><div className="overviewHead"><div><span>MAPPA DEL VIAGGIO</span><h2>Itinerario completo</h2><p>Tutte le città, le tappe e i pernottamenti disponibili lungo il viaggio.</p></div><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(experience.journey.destinationCountry)}`} target="_blank" rel="noreferrer">Apri indicazioni <ExternalLink/></a></div>{operationalPoints.length > 0 ? <><div className="overviewMap"><TripOverviewMap days={operationalPoints} onSelect={()=>undefined}/></div><div className="overviewDayList">{operationalPoints.map((entry) => <a key={`${entry.n}-${entry.lat}-${entry.lon}-${entry.title}`} href={`https://www.google.com/maps/dir/?api=1&destination=${entry.lat},${entry.lon}`} target="_blank" rel="noreferrer"><span style={{ background: entry.color }}>{entry.n}</span><span><small>Giorno {entry.n} · {entry.city}</small><strong>{entry.title}</strong></span><ExternalLink/></a>)}</div><p className="mapAttribution">Le mappe già visualizzate restano disponibili offline per un periodo limitato.</p></> : <div className="empty"><Map/><h3>Luoghi in preparazione</h3><p>Le coordinate dell’itinerario non sono ancora disponibili.</p></div>}</section>}
    ...
    {tab === "ricordi" && <section className="collection memoriesPage">
      ...
      {experience.photos.length === 0 ? <div className="empty memoriesEmpty"><Camera/><h3>La galleria aspetta il primo ricordo</h3><p>Le foto caricate nelle sfide e nei contest appariranno qui, disponibili per tutto il gruppo.</p><button type="button" onClick={() => selectTab("sfide")}>Apri le sfide</button></div> : <>
    ...
    {tab === "documenti" && <section className="collection documentsPage">
      <header className="documentsHead"><div><Wallet/><span><small>DOCUMENT WALLET</small><h2>Voucher, biglietti e documenti</h2><p>Archivio privato del tuo gruppo, consultabile anche offline dopo il download.</p></span></div><div className="offlinePackageAction"><button type="button" disabled={offlinePackage==="downloading"||offlinePackage==="ready"||!isOnline} onClick={()=>void prepareOffline()} aria-label={offlinePackage==="downloading"?`Preparazione offline ${offlineProgress.done} di ${offlineProgress.total}`:undefined}><Download/>{offlinePackage==="downloading"?"Preparazione offline…":offlinePackage==="ready"?"Disponibile offline":!isOnline?"Connettiti per scaricare":"Scarica il viaggio"}</button><small>{travelDocuments.length} {travelDocuments.length === 1 ? "documento" : "documenti"}</small></div></header>
      {travelDocuments.length === 0 ? <div className="empty documentsEmpty"><FileText/><h3>Nessun documento disponibile</h3><p>L’agenzia non ha ancora allegato documenti per il tuo gruppo. Li troverai qui appena saranno pubblicati.</p><button type="button" onClick={() => selectTab("programma")}>Torna al programma</button></div> : <div className="documentList">{travelDocuments.map((ticket) => {
