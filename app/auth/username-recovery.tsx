...
export default function UsernameRecovery({initialUsername="",initialCode="",confirmInitially=false}:{initialUsername?:string;initialCode?:string;confirmInitially?:boolean}) {
  ...
  async function confirmReset(event: FormEvent) {
    ...
    try {
      ...
      setStep("done");
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Ripristino non riuscito"); }
    finally { setBusy(false); }
  }

  if (step === "done") return <div role="status"><CheckCircle2/><h2>Password aggiornata</h2><p>Ora puoi accedere usando il tuo username e la nuova password.</p></div>;

  return <form onSubmit={step === "request" ? requestReset : confirmReset}>
    ...
    {step === "confirm" && <>
      ...
      <div className="codeInput"><KeyRound/><input id="recovery-password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={10} maxLength={256}/><button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Nascondi password" : "Mostra password"}>{showPassword ? <EyeOff/> : <Eye/>}</button></div>
