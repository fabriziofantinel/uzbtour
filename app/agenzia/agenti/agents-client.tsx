...
import { Accessibility, BarChart3, Building2, CheckCircle2, CircleAlert, LoaderCircle, LogOut, Mail, MapPinned, Phone, Plus, Trash2, UserPlus, UsersRound, X } from "lucide-react";
...
export default function AgencyAgentsClient({actor,agencies,initialAgents}:Props){
  ...
  const agencyStyle={"--agency-ui":agencyColor,"--smf-brand":agencyColor,"--smf-brand-deep":accessibleBrandColor(agencyColor)} as CSSProperties;
  ...
  async function deleteAgent(){
    if(!agency||!agentToDelete)return;setBusy(true);setError("");setNotice("");
    try{const result=await readJson<{agents:AgencyAgent[]}>(await fetch(`/api/admin/platform/agencies/${agency.id}/agents`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId:agentToDelete.id})}));
      const deletedName=agentToDelete.name;setAgentsByAgency((current)=>({...current,[agency.id]:result.agents}));setAgentToDelete(null);setNotice(`L’agente “${deletedName}” è stato eliminato dall’agenzia.`);
    }catch(caught){setError(caught instanceof Error?caught.message:"Eliminazione agente non riuscita");}finally{setBusy(false);}
  }
    ...
    <div className="agencyShell"><aside className="agencySidebar"><label htmlFor="active-agency">Agenzia attiva</label><div className="agencySelect agencySelectWithoutArrow"><Building2 size={18}/><select id="active-agency" value={agency?.id??""} onChange={(event)=>{setAgencyId(event.target.value);setError("");setNotice("");setShowForm(false);}}>{agencies.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div><nav><Link href="/agenzia"><MapPinned size={18}/> Viaggi</Link><Link className="active" href="/agenzia/agenti" aria-current="page"><UsersRound size={18}/> Agenti</Link><Link href="/agenzia/analytics"><BarChart3 size={18}/> Analytics</Link><Link href="/accessibilita"><Accessibility size={18}/> Accessibilità</Link></nav></aside>
      <section id="main-content" className="agencyContent" tabIndex={-1}>{error&&<div className="agencyMessage error" role="alert"><CircleAlert size={18}/><span>{error}</span></div>}{notice&&<div className="agencyMessage success" role="status"><CheckCircle2 size={18}/><span>{notice}</span></div>}
        <section className="agencySection agentManagementSection"><div className="agencySectionHead"><div><h2>Elenco agenti</h2><p>{agents.length} {agents.length===1?"agente censito":"agenti censiti"}</p></div><div className="agencySectionActions"><button type="button" className="newTripButton" aria-expanded={showForm} aria-controls="new-agent-form" onClick={()=>{setShowForm((value)=>!value);setError("");}}><Plus size={17}/> Nuovo agente</button></div></div>
          ...
          <div className="agencyAgentList">{agents.map((agent)=><article key={agent.id}><i>{agent.name.split(/\s+/).slice(0,2).map((part)=>part[0]).join("").toUpperCase()}</i><div><h3>{agent.name}</h3><p>@{agent.username}</p><span><a href={`mailto:${agent.email}`}><Mail/> {agent.email}</a><b><Phone/> {agent.phone||"Telefono non indicato"}</b></span></div><strong className={agent.status}>{agent.status==="active"?"Attivo":agent.status==="invited"?"Invito inviato":"Sospeso"}</strong><button type="button" className="agencyAgentDelete" aria-label={`Elimina ${agent.name}`} onClick={()=>{setError("");setAgentToDelete(agent);}}><Trash2/> Elimina</button></article>)}{!agents.length&&<div className="agencyEmpty"><UsersRound/><h3>Nessun agente</h3><p>Usa “Nuovo agente” per condividere la gestione dei viaggi.</p></div>}</div>
        </section></section></div>
    {agentToDelete&&<div className="deleteTripBackdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!busy)setAgentToDelete(null);}}><section className="deleteTripDialog" role="dialog" aria-modal="true" aria-labelledby="delete-agent-title"><button type="button" className="deleteTripClose" aria-label="Chiudi" disabled={busy} onClick={()=>setAgentToDelete(null)}><X/></button><i><Trash2/></i><small>OPERAZIONE DEFINITIVA</small><h2 id="delete-agent-title">Eliminare “{agentToDelete.name}”?</h2><p>L’agente perderà immediatamente l’accesso all’agenzia. I dati storici e le attività già registrate resteranno disponibili.</p>{error&&<p className="dialogInlineError" role="alert"><CircleAlert/>{error}</p>}<div><button type="button" className="secondary" disabled={busy} onClick={()=>setAgentToDelete(null)}>Annulla</button><button type="button" className="danger" disabled={busy} onClick={()=>void deleteAgent()}>{busy?<><LoaderCircle className="spin"/> Eliminazione…</>:<><Trash2/> Elimina definitivamente</>}</button></div></section></div>}
  </main>;
}
