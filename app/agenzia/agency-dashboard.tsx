...
export default function AgencyDashboard({ initialOverview }: Props) {
  ...
  const tripPeriodCounts = useMemo(() => {
    ...
    return allDepartures.reduce(
      (counts, { trip, departure }) => {
  ...
  const filteredDepartures = useMemo(() => {
    ...
    return rows.sort((left, right) => {
      if (tripSort === "name")
        return (left.departure?.title || left.trip.title).localeCompare(
          right.departure?.title || right.trip.title,
          "it",
          { sensitivity: "base" },
        );
  ...
  async function refresh() {
    const result = await responseJson<PlatformOverview>(
      await fetch("/api/admin/platform/overview", {
        cache: "no-store",
      }),
    );
    setOverview(result);
  ...
  async function createTrip(event: FormEvent<HTMLFormElement>) {
    ...
    if (!(file instanceof File) || file.size === 0) {
      setError("Seleziona il preventivo accettato dal cliente in formato PDF, DOC o DOCX.");
      setBusy("");
      return;
    }
    if (!travelDocumentType(file.name)) {
      setError("Il preventivo deve essere in formato PDF, DOC o DOCX.");
      setBusy("");
      return;
    }
    ...
    try {
      ...
      const created = await responseJson<{ trip: { id: string } }>(
        await fetch("/api/admin/platform/trips", {
          method: "POST",
  ...
  async function uploadProgramme(templateId: string, file: File) {
    ...
    try {
      const authorization = await responseJson<UploadAuthorization>(
        await fetch("/api/admin/platform/documents/upload", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            agencyId: agency.id,
      ...
      await responseJson(
        await fetch("/api/admin/platform/documents", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            agencyId: agency.id,
  ...
  async function processImport(importId: string) {
    ...
    setNotice("");
    try {
      const result = await responseJson<{ import: { status: string; days?: number } }>(
        await fetch(`/api/admin/platform/imports/${importId}/process`, { method: "POST" }),
      );
      await refresh();
      setNotice(
        result.import.days
  ...
  async function retryEnrichment(templateId: string) {
    ...
    try {
      await responseJson(
        await fetch(`/api/admin/platform/trips/${templateId}/enrichment`, {
          method: "POST",
        }),
      );
      await refresh();
  ...
  async function deleteTrip() {
    ...
    setNotice("");
    try {
      await responseJson<{ ok: true; deletedFiles: number }>(
        await fetch(`/api/admin/platform/trips/${tripToDelete.id}`, { method: "DELETE" }),
      );
  ...
  return (
    <main className="agencyPage" style={agencyStyle}>
      ...
      <section className="agencyHero">
        <div>
          <p>
            <Sparkles size={15} /> PIATTAFORMA VIAGGI
          </p>
        ...
        <section id="main-content" className="agencyContent" tabIndex={-1}>
          ...
          <section id="viaggi" className="agencySection">
            ...
            {showNewTrip && (
              <form id="new-trip-form" className="newTripForm" onSubmit={createTrip} aria-busy={busy === "new-trip"}>
                <div className="formIntro">
                  <b>Crea il viaggio dal preventivo</b>
                  <span>
                    Carica il documento accettato dal cliente. L’app preparerà testata, itinerario e anagrafiche per la
                    tua revisione.
                  </span>
                </div>
                <ol className="newTripWorkflow" aria-label="Fasi di creazione del viaggio">
                  ...
                  <li>
                    <b>2</b>
                    <span>
                      <strong>Attendi</strong>
                      <small>Estrazione automatica</small>
                    </span>
                  </li>
                  <li>
                    <b>3</b>
                    <span>
                      <strong>Revisiona</strong>
                      <small>Controllo prima di pubblicare</small>
                    </span>
                  </li>
                </ol>
                <div className="quoteTemplate">
                  <Download />
                  <span>
                    <b>Modello preventivo SMF Travel</b>
                    <small>Usalo per ridurre gli errori di interpretazione.</small>
                  </span>
                  <a href="/templates/modello-preventivo-smf-travel.docx" download>
                    Scarica DOCX
                  </a>
                </div>
                <div className="newTripFields">
                  <label htmlFor="trip-title">
                    Nome pratica <span>(facoltativo)</span>
                    <input
                      id="trip-title"
                      name="title"
                      maxLength={160}
                      autoComplete="off"
                      placeholder="Se vuoto useremo il nome del file"
                    />
                  </label>
                  <label
                    className={`quoteUploadField ${selectedProgrammeName ? "selected" : ""}`}
                    htmlFor="trip-programme"
                  >
                    <input
                      id="trip-programme"
                      name="programme"
                      type="file"
                      aria-describedby="trip-programme-help"
                      accept="application/pdf,.pdf,application/msword,.doc,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.docx"
                      required
                      onChange={(event) => setSelectedProgrammeName(event.target.files?.[0]?.name ?? "")}
                    />
                    <span className="quoteUploadIcon">{selectedProgrammeName ? <FileCheck2 /> : <Plus />}</span>
                    <span>
                      <b>{selectedProgrammeName || "Seleziona il preventivo"}</b>
                      <small id="trip-programme-help">PDF, DOC o DOCX · massimo 20 MB</small>
                    </span>
                ...
                <div className="newTripActions">
                  ...
                  <button type="submit" disabled={busy === "new-trip"}>
                    {busy === "new-trip" ? (
                      <>
                        <LoaderCircle className="spin" /> Creazione…
                      </>
                    ) : (
                      <>Crea e avvia analisi</>
                    )}
                  </button>
                </div>
              </form>
            ...
            <div className="tripFilters" role="group" aria-label="Filtra i viaggi">
              ...
              <button
                type="button"
                aria-pressed={tripPeriod === "all"}
                className={tripPeriod === "all" ? "active" : ""}
                onClick={() => setTripPeriod("all")}
              >
                Tutti <b>{tripPeriodCounts.all}</b>
              </button>
              <button
                type="button"
                aria-pressed={tripPeriod === "upcoming"}
                className={tripPeriod === "upcoming" ? "active" : ""}
                onClick={() => setTripPeriod("upcoming")}
              >
                Da fare <b>{tripPeriodCounts.upcoming}</b>
              </button>
              <button
                type="button"
                aria-pressed={tripPeriod === "ongoing"}
                className={tripPeriod === "ongoing" ? "active" : ""}
                onClick={() => setTripPeriod("ongoing")}
              >
                In corso <b>{tripPeriodCounts.ongoing}</b>
              </button>
              <button
                type="button"
                aria-pressed={tripPeriod === "past"}
                className={tripPeriod === "past" ? "active" : ""}
                onClick={() => setTripPeriod("past")}
              >
                Fatti <b>{tripPeriodCounts.past}</b>
              </button>
              <div className="tripSearch">
                <Search />
                <label className="srOnly" htmlFor="traveler-filter">
            ...
            <div className="tripListTools">
              <p className="tripResultCount" aria-live="polite">
                ...
                {filteredDepartures.length === 1 ? "viaggio visualizzato" : "viaggi visualizzati"}
              </p>
              {(tripPeriod !== "all" || travelerFilter) && (
                <button type="button" className="clearAllTripFilters" onClick={clearTripFilters}>
                  <X /> Azzera filtri
                </button>
              )}
              <label htmlFor="trip-sort">
                Ordina per
                <select
                  id="trip-sort"
                  value={tripSort}
                  onChange={(event) => setTripSort(event.target.value as typeof tripSort)}
                >
              ...
              <div className="tripTableWrap">
                <table className="tripTable">
                  ...
                  <tbody>
                    {filteredDepartures.map(({ trip, departure }) => {
                      ...
                      return (
                        <tr
                          className={importIsActive ? "generating" : ""}
                          key={`${trip.id}-${departure?.id ?? "draft"}`}
                        >
                          <td data-label="Stato">
                            <span className={`status ${displayStatus.className}`}>{displayStatus.label}</span>
                          </td>
                          <td data-label="Viaggio" className="tripNameCell">
                            <b>{departure?.title || trip.title}</b>
                            <span>{trip.destinationCountry || "Destinazione da revisionare"}</span>
                          </td>
                          <td data-label="Periodo" className="dateRangeCell">
                            <b>{formatTravelDate(startsOn)}</b>
                            <span aria-hidden="true">→</span>
                            <b>{formatTravelDate(endsOn)}</b>
                          </td>
                          <td data-label="Gruppi" className="numberCell">
                            {departure?.partyCount ?? 0}
                          </td>
                          <td data-label="Azioni">
                            <div className="tableActions inline">
                              {latestImport?.status === "ready_for_review" && (
                                <Link className="primary" href={`/agenzia/importazioni/${latestImport.id}`}>
                                  <Eye /> Revisiona
                                </Link>
                              )}
                              {departure && (
                ...
                {filteredDepartures.length === 0 && (
                  <div className="agencyEmpty">
                    <MapPinned />
                    <h3>{agency?.trips.length ? "Nessun risultato" : "Nessun viaggio ancora"}</h3>
                    <p>
                      {agency?.trips.length
                        ? "Modifica i filtri per visualizzare altri viaggi."
                        : "Usa Nuovo viaggio quando vuoi importare un preventivo accettato."}
                    </p>
                    {agency?.trips.length ? (
                      <button type="button" onClick={clearTripFilters}>
                        Azzera filtri
                      </button>
                    ) : null}
                  </div>
            ...
            {tripView === "cards" && (
              <div className="tripAdminGrid cards">
                {filteredDepartures.map(({ trip, departure }) => {
                  ...
                  return (
                    <article
                      className={`tripAdminCard journeyCard ${importIsActive || contentIsActive ? "generating" : ""}`}
                      key={`${trip.id}-${departure?.id ?? "draft"}`}
                    >
                      ...
                      <p>
                        {trip.destinationCountry || "Destinazione da revisionare"} · Programma: {trip.title}
                      </p>
                      ...
                      <div className="journeyCardFacts">
                        <span>
                          <UsersRound />
                          <small>GRUPPI</small>
                          <b>{departure?.partyCount ?? 0}</b>
                        </span>
                        <span>
                          <UsersRound />
                          <small>VIAGGIATORI</small>
                          <b>{departure?.travelerNames.length ?? 0}</b>
                        </span>
                      </div>
                      <div className="journeyTravelers">
                        <small>PARTECIPANTI</small>
                        <p>{departure?.travelerNames.join(", ") || "Nessun viaggiatore configurato"}</p>
                      </div>
                          ...
                          <div className={`tripGeneration ${latestImport.status}`}>
                            <div>
                              ...
                              <span>
                                <b>
                                  {importIsActive
                                    ? "Viaggio in generazione"
                                    : (statusLabels[latestImport.status] ?? latestImport.status)}
                                </b>
                                <small>
                                  {latestImport.status === "ready_for_review"
                                    ? "Il programma è pronto per il controllo."
                                    : latestImport.status === "failed"
                                      ? "Analisi non riuscita: puoi riprovare."
                                      : "Documento e itinerario acquisiti."}
                                </small>
                              </span>
                            </div>
                            {importIsActive && (
                              <i
                                ...
                                aria-valuemin={0}
                                aria-valuemax={100}
                                aria-valuenow={importProgress(latestImport.status)}
                              >
                                <span style={{ width: `${importProgress(latestImport.status)}%` }} />
                              </i>
                            )}
                            {latestImport.status === "failed" && (
                              <button
                                type="button"
                                disabled={Boolean(busy)}
                                onClick={() => void processImport(latestImport.id)}
                              >
                                {busy === `process-${latestImport.id}` ? <LoaderCircle className="spin" /> : <Play />}{" "}
                                Riprova
                              </button>
                            )}
                            {latestImport.status === "ready_for_review" && (
                              <a href={`/agenzia/importazioni/${latestImport.id}`}>
                                <Eye /> Revisiona
                              </a>
                            )}
                          </div>
                          ...
                          <div className={`tripGeneration ${contentIsComplete ? "published" : content.status}`}>
                            <div>
                              ...
                              <span>
                                ...
                                <small>
                                  {content.readySections}/{content.expectedSections} sezioni pronte tra info, frasi,
                                  quiz, missioni, giochi e contest.
                                </small>
                                {content.contestTitles.length > 0 && (
                                  <small>Contest: {content.contestTitles.slice(0, 2).join(" · ")}</small>
                                )}
                              </span>
                            </div>
                            {contentIsActive && content.expectedSections > 0 && (
                              <i
                                role="progressbar"
                                aria-label="Avanzamento generazione contenuti"
                                aria-valuemin={0}
                                aria-valuemax={content.expectedSections}
                                aria-valuenow={content.readySections}
                              >
                                <span
                                  style={{
                                    width: `${Math.max(8, Math.round((content.readySections / content.expectedSections) * 100))}%`,
                                  }}
                                />
                              </i>
                            )}
                            {content.status === "failed" && (
                              <button
                                type="button"
                                disabled={Boolean(busy)}
                                onClick={() => void retryEnrichment(trip.id)}
                              >
                                {busy === `enrichment-${trip.id}` ? <LoaderCircle className="spin" /> : <Play />}{" "}
                                Riprova contenuti
                              </button>
                            )}
                          </div>
      ...
      {tripToDelete && (
        ...
        >
          <section className="deleteTripDialog" role="dialog" aria-modal="true" aria-labelledby="delete-trip-title">
            ...
            <p>
              Verranno eliminati programma, importazioni, partenze, gruppi e file collegati. Le anagrafiche condivise e
              gli utenti resteranno disponibili.
            </p>
            {error && (
            ...
            <div>
              ...
              <button type="button" className="danger" disabled={Boolean(busy)} onClick={() => void deleteTrip()}>
                {busy === `delete-${tripToDelete.id}` ? (
                  <>
