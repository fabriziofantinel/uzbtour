...
export default function ProgrammeEditor({ initialProgramme, editableDayIds }: Props) {
  ...
  const [expandedItems, setExpandedItems] = useState(
    () => new Set(initialProgramme.days.flatMap((day) => (day.items[0] ? [day.items[0].id] : []))),
  );
  ...
  async function cancelItem(day: Day, item: Item) {
    ...
    try {
      await responseJson(
        await fetch(`/api/admin/platform/departures/${departure.id}/programme`, {
          method: "PATCH",
  ...
  async function saveDay(day: Day) {
    ...
    try {
      ...
      setMessage({
        kind: "error",
        text: error instanceof Error ? error.message : "Salvataggio non riuscito. Riprova senza perdere le modifiche.",
      });
    } finally {
      setBusy("");
    }
  ...
  async function uploadTicket(dayId: string, itemId: string, file: File) {
    ...
    if (file.size > 25 * 1024 * 1024) {
      setMessage({ kind: "error", text: "Il biglietto supera 25 MB. Scegli un file più piccolo." });
      return;
    }
    if (!allowedTypes.has(file.type)) {
      setMessage({ kind: "error", text: "Formato non supportato. Carica un PDF oppure un’immagine JPG, PNG o WebP." });
      return;
    }
    setBusy(`ticket-${itemId}`);
    setMessage(null);
    try {
      ...
      const result = (await response.json().catch(() => ({}))) as {
        error?: string;
        ticket?: Day["items"][number]["tickets"][number];
      };
          ...
          day.id !== dayId
            ...
            : {
                ...
                items: day.items.map((item) =>
                  item.id === itemId ? { ...item, tickets: [...item.tickets, result.ticket!] } : item,
                ),
              },
        ),
      );
      ...
      setMessage({
        kind: "error",
        text: error instanceof Error ? error.message : "Caricamento del biglietto non riuscito. Riprova.",
      });
    } finally {
      setBusy("");
    }
