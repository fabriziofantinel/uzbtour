...
export default function JourneyTravelers({ initialData }: { initialData: JourneyData }) {
  ...
  async function removeTraveler(partyId: string, travelerId: string, name: string) {
    if (
      !(await confirmAction({
        title: "Rimuovi il viaggiatore",
        message: `${name} perderà l’accesso a questo viaggio e ai contenuti del gruppo.`,
        confirmLabel: "Rimuovi viaggiatore",
        tone: "danger",
      }))
    )
      return;
  ...
  async function removeGroup(partyId: string, name: string) {
    if (
      !(await confirmAction({
        ...
        message: `Il gruppo ${name} sarà eliminato definitivamente. Per procedere deve essere vuoto.`,
    ...
    <>
      <main className="journeyManagePage" style={agencyStyle}>
        ...
        <section className="journeyManageHero">
          ...
          <p>
            <CalendarDays /> {formatDate(data.journey.startsOn)} – {formatDate(data.journey.endsOn)} ·{" "}
            {data.journey.code}
          </p>
        </section>
        <div className="journeyManageShell">
          ...
          {familyForm && (
            <form
              className="familyCreateForm"
              id="family-create-form"
              onSubmit={addFamily}
              aria-busy={busy === "family"}
            >
              <div>
