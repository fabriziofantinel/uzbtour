...
import { ArrowLeft, BookOpen, CalendarDays, Check, CheckCircle2, CircleAlert, Copy, Crown, FolderOpen, LoaderCircle, Mail, MessageCircle, Plus, ShieldCheck, Trash2, Trophy, UserPlus, UsersRound, X } from "lucide-react";
...
export default function JourneyTravelers({ initialData }: { initialData: JourneyData }) {
  ...
  async function updateMinorConsent(partyId:string,travelerId:string,decision:"granted"|"denied"|"withdrawn"){
    setBusy(`consent-${travelerId}`);setError("");setNotice("");
    try{const result=await json<{data:JourneyData}>(await fetch(`/api/admin/platform/trips/${data.journey.id}/families/${partyId}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"minorConsent",agencyId:data.journey.agencyId,travelerId,decision})}));setData(result.data);setNotice("Consenso immagini aggiornato e registrato nell’audit.");}
    catch(caught){setError(caught instanceof Error?caught.message:"Consenso non aggiornato");}finally{setBusy("");}
  }
  async function removeTraveler(partyId:string,travelerId:string,name:string){if(!window.confirm(`Rimuovere ${name} da questo viaggio?`))return;setBusy(`delete-${travelerId}`);setError("");try{const result=await json<{data:JourneyData}>(await fetch(`/api/admin/platform/trips/${data.journey.id}/families/${partyId}`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({agencyId:data.journey.agencyId,travelerId})}));setData(result.data);setNotice("Viaggiatore rimosso dal viaggio.");}catch(caught){setError(caught instanceof Error?caught.message:"Eliminazione non riuscita");}finally{setBusy("");}}
  async function removeGroup(partyId:string,name:string){if(!window.confirm(`Eliminare definitivamente il gruppo ${name}? Il gruppo deve essere vuoto.`))return;setBusy(`delete-group-${partyId}`);setError("");try{const result=await json<{data:JourneyData}>(await fetch(`/api/admin/platform/trips/${data.journey.id}/families/${partyId}`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({agencyId:data.journey.agencyId})}));setData(result.data);setNotice("Gruppo eliminato.");}catch(caught){setError(caught instanceof Error?caught.message:"Eliminazione non riuscita");}finally{setBusy("");}}
    ...
    <div className="journeyManageShell">
      ...
      <section className="familyCards">
        {data.families.map((family) => { const leader = family.travelers.find((traveler) => traveler.role === "organizer"); return <article key={family.id}>
          <header><span><UsersRound/><b>{family.name}</b><small>{family.travelers.length} {family.travelers.length === 1 ? "viaggiatore" : "viaggiatori"} · {family.code}</small></span><div className="groupHeaderActions"><span className={leader ? "groupLeaderSummary" : "groupLeaderSummary missing"}><Crown/><small>Capogruppo</small><b>{leader?.name ?? "Da indicare"}</b></span><button type="button" aria-expanded={travelerFamily === family.id} aria-controls={`traveler-form-${family.id}`} onClick={() => { setTravelerFamily(travelerFamily === family.id ? "" : family.id); setError(""); }}><UserPlus/> {travelerFamily === family.id ? "Chiudi inserimento" : "Aggiungi viaggiatore"}</button><button type="button" className="dangerAction" disabled={Boolean(busy) || family.travelers.length > 0} title={family.travelers.length > 0 ? "Rimuovi prima tutti i viaggiatori" : "Elimina gruppo"} onClick={() => void removeGroup(family.id, family.name)}><Trash2/> Elimina gruppo</button></div></header>
          ...
          <div className="familyTravelers">{family.travelers.map((traveler) => <div key={traveler.id} className={traveler.role === "organizer" ? "isLeader" : ""}><i>{traveler.name.split(/\s+/).slice(0, 2).map((part) => part[0] || "").join("").toUpperCase()}</i><span><b>{traveler.name}</b><small>@{traveler.username} · <Mail/> {traveler.email}</small></span>{traveler.memberType==="dependent_minor"&&<label className="minorConsentControl"><ShieldCheck/><span><b>Consenso immagini</b><small>Decisione del genitore o tutore</small></span><select value={traveler.minorImageConsent} disabled={Boolean(busy)} onChange={(event)=>void updateMinorConsent(family.id,traveler.id,event.currentTarget.value as "granted"|"denied"|"withdrawn")}><option value="missing" disabled>Da acquisire</option><option value="granted">Concesso</option><option value="denied">Negato</option>{traveler.minorImageConsent==="granted"&&<option value="withdrawn">Revoca</option>}</select></label>}<label className="travelerCompetitionSwitch"><input type="checkbox" checked={traveler.participatesInTripGames} disabled={Boolean(busy)} onChange={(event) => void updateCompetition(family.id, traveler.id, event.currentTarget.checked)}/><Trophy/><span><b>Giochi a livello viaggio</b><small>{traveler.participatesInTripGames ? "Partecipa" : "Solo gruppo"}</small></span></label>{traveler.role === "organizer" ? <em className="leaderBadge"><Crown/> Capogruppo</em> : <button type="button" className="setLeaderButton" disabled={Boolean(busy)} onClick={() => void setLeader(family.id, traveler.id)}><Crown/> Imposta capogruppo</button>}<strong className={traveler.status}>{traveler.status === "invited" ? "Da attivare" : traveler.status === "active" ? "Attivo" : traveler.status}</strong><button type="button" className="travelerDelete" disabled={Boolean(busy) || (traveler.role === "organizer" && family.travelers.length > 1)} title={traveler.role === "organizer" && family.travelers.length > 1 ? "Imposta prima un altro capogruppo" : `Rimuovi ${traveler.name}`} aria-label={`Rimuovi ${traveler.name} dal viaggio`} onClick={() => void removeTraveler(family.id, traveler.id, traveler.name)}><Trash2/> Elimina</button>{activation?.travelerId === traveler.id && <div className="travelerActivationLink"><ShieldCheck/><span><b>Link personale di attivazione</b><small>Monouso, valido 14 giorni.</small></span><label htmlFor={`activation-link-${traveler.id}`}>Link<input id={`activation-link-${traveler.id}`} readOnly value={activation.url} onFocus={(event) => event.currentTarget.select()}/></label><button type="button" className={activationCopied ? "copied" : ""} onClick={() => void copyActivationLink()}>{activationCopied ? <Check/> : <Copy/>} {activationCopied ? "Copiato" : "Copia"}</button><button type="button" className="activationDismiss" aria-label={`Nascondi link di ${traveler.name}`} onClick={() => { setActivation(null); setActivationCopied(false); }}><X/></button></div>}</div>)}{family.travelers.length === 0 && <div className="familyTravelersEmpty"><UserPlus/><span><strong>Nessun viaggiatore</strong><small>Aggiungi per primo un adulto: diventerà il capogruppo.</small></span></div>}</div>
        </article>; })}
