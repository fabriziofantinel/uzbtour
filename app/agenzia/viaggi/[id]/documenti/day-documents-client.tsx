...
export default function DayDocumentsClient({ initialData }: { initialData: AgencyDayDocuments }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const dayId = String(data.get("dayId") || ""); const partyId = String(data.get("partyId") || ""); const description = String(data.get("description") || "").trim();
    const file = data.get("document");
    if (!(file instanceof File) || !file.size || !dayId || !partyId || !description) { setMessage({ kind:"error", text:"Seleziona giornata e gruppo, inserisci la descrizione e scegli un documento." }); return; }
    setBusy(true); setMessage(null);
    try {
      const uploaded = await uploadPrivateFile({ endpoint:`/api/admin/platform/departures/${departure.id}/day-documents/upload`, file, payload:{ dayId, partyId } });
      const response = await fetch(`/api/admin/platform/departures/${departure.id}/day-documents`, { method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ dayId, partyId, description, objectKey:uploaded.key, originalName:file.name, contentType:uploaded.contentType }) });
      ...
      setDocuments((current)=>[result.document!,...current]); form.reset(); setMessage({kind:"success",text:"Documento associato alla giornata e al gruppo."});
  ...
  return <main className="journeyManagePage dayDocumentsPage" style={{"--smf-brand":departure.agencyPrimaryColor,"--smf-brand-deep":departure.agencyPrimaryColor,"--smf-action":departure.agencyPrimaryColor} as CSSProperties}>
    ...
    <div className="journeyManageShell">
      ...
      <form className="dayDocumentForm" onSubmit={submit} aria-busy={busy}>
        ...
        <label>Gruppo *<select name="partyId" required defaultValue=""><option value="" disabled>Seleziona il gruppo</option>{initialData.groups.map((group)=><option key={group.id} value={group.id}>{group.name} · {group.code}</option>)}</select></label>
