...
export default function AgencyImpersonationClient({
  users,
  primaryColor,
}: {
  ...
  const filtered = useMemo(
    () =>
      users.filter((user) =>
        [user.name, user.username, user.email, ...user.departureTitles]
          .join(" ")
          .toLowerCase()
  ...
  return (
    <main className="agencyLoginAs" style={style}>
      <header>
        <a href="/agenzia">
          <ArrowLeft /> Pannello agenzia
        </a>
        <div>
      ...
      {selected && (
        <div
          className="agencyLoginOverlay"
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setSelected(null);
          }}
        >
          <div role="dialog" aria-modal="true">
            <button className="close" aria-label="Chiudi" onClick={() => setSelected(null)}>
              <X />
            </button>
