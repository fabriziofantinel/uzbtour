...
export async function POST(request:Request){
  ...
  if(!user.authSubject)return NextResponse.json({error:"Gli avvisi non possono essere attivati durante una sessione Login come"},{status:403});
  ...
  const rows=await getSql()`SELECT app.upsert_own_web_push_subscription_v3(${user.authSubject},${parsed.data.endpoint},${parsed.data.keys.p256dh},${parsed.data.keys.auth},${request.headers.get("user-agent")||""},${expiration}::timestamptz) id`;
