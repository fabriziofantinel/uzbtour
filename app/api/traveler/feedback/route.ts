...
export async function POST(request: Request) {
  try {
    ...
    const clientOperationId = body?.clientOperationId == null ? crypto.randomUUID() : String(body.clientOperationId);
    if (
      !/^[0-9a-f-]{36}$/i.test(clientOperationId) ||
      !["itinerary_item", "hotel"].includes(targetType) ||
