...
import { readV3ChallengeAnswerSpecs } from "@/lib/platform/v3-gamification";
import {
...
export async function POST(request: Request) {
  ...
  if (body?.action === "confirmPhotoContest") {
    ...
    try {
      ...
      const queued = await getJobQueue().enqueue({
        ...
        payload: { departureId, partyId, itemId: contentId, entryIds },
      });
      return NextResponse.json({ id: queued.id, status: "evaluating", entryIds });
    } catch (error) {
      if (error instanceof Error && /exactly two/i.test(error.message))
        return NextResponse.json({ error: "Carica due foto prima di confermare" }, { status: 409 });
      if (error instanceof Error && /closed/i.test(error.message))
        return NextResponse.json({ error: "Il contest è già chiuso" }, { status: 409 });
      throw error;
    }
  }
  if (body?.action === "photoEvidence") {
    ...
    if (contentType === "photo_contest") {
      try {
        ...
        if (error instanceof Error && /no longer editable/i.test(error.message))
          return NextResponse.json({ error: "Le foto sono già state confermate" }, { status: 409 });
        if (error instanceof Error && /contest is closed/i.test(error.message))
          return NextResponse.json({ error: "Il contest è già chiuso" }, { status: 409 });
        throw error;
      }
    }
    ...
    try {
      ...
      if (error instanceof Error && /validation pending/i.test(error.message))
        return NextResponse.json({ error: "La foto è ancora in verifica" }, { status: 409 });
      if (error instanceof Error && /already approved/i.test(error.message))
        return NextResponse.json({ error: "La missione è già stata superata" }, { status: 409 });
      if (error instanceof Error && /attempt limit reached/i.test(error.message))
        return NextResponse.json({ error: "Hai esaurito i due tentativi disponibili" }, { status: 409 });
      throw error;
    }
      ...
      return NextResponse.json(
        {
          id: resultId,
          status: "submitted",
          aiValidation: "queue_failed",
          error: "Foto registrata; validazione AI temporaneamente in attesa",
        },
        { status: 503 },
      );
    }
  ...
  if (body?.action === "reviewEvidence") {
    if (!user.isAgencyAdmin)
      return NextResponse.json({ error: "Solo l’amministratore può validare le foto" }, { status: 403 });
  ...
  if (["game", "puzzle", "memory", "oddOneOut", "city", "visitCount"].includes(gameAction)) {
    ...
    if (!v3Items[0] || !["word_game", "order_game", "puzzle"].includes(String(v3Items[0].activity_type))) {
      return NextResponse.json({ error: "Gioco non disponibile nel catalogo pubblicato" }, { status: 404 });
    }
    const answerSpec =
      v3Items[0]?.answer_spec && typeof v3Items[0].answer_spec === "object" && !Array.isArray(v3Items[0].answer_spec)
        ? (v3Items[0].answer_spec as Record<string, unknown>)
        : {};
    const expected =
      gameAction === "oddOneOut" ? String(answerSpec.correctIndex ?? "") : String(answerSpec.answer || "").trim();
    const correct =
      gameAction === "puzzle" ||
