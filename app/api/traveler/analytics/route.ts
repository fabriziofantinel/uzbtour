...
const inputSchema=z.object({
  departureId:z.string().uuid(),partyId:z.string().uuid(),dayId:z.string().uuid().nullable().optional(),
  eventName:z.enum(["traveler_session","programme_view","document_list_view","document_download"]),
  sessionId:z.string().uuid(),clientOperationId:z.string().uuid(),
...
export async function POST(request:Request){
  ...
  }catch(error){return platformApiError(error,"Analytics non disponibile");}
}
