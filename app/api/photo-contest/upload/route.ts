...
async function assertContestOpen(day: number, contestType: PhotoContestType, userId: string) {
  ...
  if (closed.length > 0) {
    throw new Error(
      String(closed[0].status) === "processing" ? "La giuria del contest è già in corso" : "Il contest è già concluso",
    );
  }
...
export async function POST(request: Request) {
  ...
  const limited = await enforceApiRateLimit(
    request,
    { scope: "upload.photo-contest", limit: 30, windowSeconds: 600 },
    user.id,
  );
