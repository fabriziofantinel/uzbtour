...
const brandingSchema = z.object({
  ...
  logoUrl: z.union([
    z.literal(""),
    z.url(),
    z.string().regex(/^r2:\/\/agencies\/[0-9a-f-]{36}\/branding\/[0-9a-f-]{36}\.(png|jpe?g|webp)$/i),
  ]),
});
const statusSchema = z.object({ action: z.literal("status"), status: z.enum(["trial", "active", "suspended"]) });
const ownerSchema = z.object({
  action: z.literal("replace-owner"),
...
const ownerContactSchema = z.object({
  action: z.literal("update-owner-contact"),
  email: z.email(),
  phone: z.string().trim().min(5).max(40),
});
const agencyDetailsSchema = z.object({
  ...
  taxCode: z.string().trim().max(32),
  registeredAddress: z.string().trim().max(240),
  registeredCity: z.string().trim().max(120),
  registeredPostalCode: z.string().trim().max(16),
  registeredProvince: z.string().trim().max(80),
  registeredCountry: z.string().trim().max(100),
  pec: z.union([z.literal(""), z.email()]),
  sdiCode: z.string().trim().max(16),
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (owner.success) {
      ...
      if (result.activationToken) {
        const activationUrl = new URL(
          `/attiva-account#token=${encodeURIComponent(result.activationToken)}`,
          request.url,
        ).toString();
    ...
    if (
      branding.data.logoUrl.startsWith("r2://") &&
      !branding.data.logoUrl.startsWith(`r2://agencies/${id}/branding/`)
    ) {
      return NextResponse.json({ error: "Logo non associato all’agenzia" }, { status: 400 });
    }
