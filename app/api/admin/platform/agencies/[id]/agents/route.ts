...
export async function POST(
  ...
  try {
    ...
    const parsed=z.object({name:z.string().trim().min(2).max(160),
      ...
      email:z.email(),phone:z.string().trim().min(5).max(40)}).safeParse(await request.json().catch(()=>null));
    if(!parsed.success)return NextResponse.json({error:"Compila nome, username, email e telefono."},{status:400});
    const created=await createAgencyAgent({actorId:actor.id,agencyId:id,...parsed.data});
    let invitationEmailSent=false;
    if(created.activationToken){const activationUrl=new URL(`/attiva-account#token=${encodeURIComponent(created.activationToken)}`,request.url).toString();
  ...
  } catch(error){return platformApiError(error,"Inserimento agente non riuscito");}
}

export async function DELETE(request:Request,context:{params:Promise<{id:string}>}){
  try{
    const actor=await requirePlatformAdmin();const {id}=await context.params;
    if(!/^[0-9a-f-]{36}$/i.test(id))return NextResponse.json({error:"Agenzia non valida"},{status:400});
  ...
  }catch(error){return platformApiError(error,"Eliminazione agente non riuscita");}
}
