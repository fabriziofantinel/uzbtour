...
import { deleteJourneyGroup, getJourneyManagement, removeJourneyTraveler, setJourneyGroupLeader, setMinorImageConsent, updateJourneyGroupCompetition } from "@/lib/platform/journey-repository";
...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("minorConsent"), agencyId: z.string().uuid(), travelerId: z.string().uuid(), decision:z.enum(["granted","denied","withdrawn"]) }),
]);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; partyId: string }> }) {
  try {
    ...
    if(input.action==="minorConsent"){
      ...
      if(!traveler||traveler.memberType!=="dependent_minor")return NextResponse.json({error:"Il consenso è previsto solo per un minore del gruppo"},{status:400});
      await setMinorImageConsent({actorId:actor.id,agencyId:input.agencyId,departureId:id,partyId,travelerId:input.travelerId,decision:input.decision});
