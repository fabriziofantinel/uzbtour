...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("competition"), agencyId: z.string().uuid(), enabled: z.boolean() }),
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; partyId: string }> }) {
  try {
    ...
    if (input.action === "leader") {
      ...
      await updateJourneyGroupCompetition({ actorId: actor.id, agencyId: input.agencyId, departureId: id, partyId, enabled: input.enabled });
    }
