...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const ticket = await registerV3TicketUpload({
      userId: actor.id, departureId, itemId, mediaId, documentId,
      provider: storage.provider, bucket: storage.bucket, objectKey,
      originalName: file.originalName, contentType: object.contentType,
      sizeBytes: object.sizeBytes,
    });
    return NextResponse.json({ ticket: {
      id: ticket.id, title: ticket.title, contentType: object.contentType,
      sizeBytes: object.sizeBytes, createdAt: ticket.createdAt,
