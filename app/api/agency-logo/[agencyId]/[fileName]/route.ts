...
export async function GET(request:Request,context:{params:Promise<{agencyId:string;fileName:string}>}){
  try{
    ...
    if(request.headers.get("x-smf-offline-package")==="1"){
      ...
      return new Response(Buffer.from(object.bytes),{headers:{"Content-Type":contentType,"Cache-Control":"private, max-age=0, must-revalidate","X-Content-Type-Options":"nosniff"}});
    }
