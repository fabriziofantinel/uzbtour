...
function validChallenge(payload: UploadPayload) {
  ...
  if (payload.type === "mission") {
    return missionDays.some(
      (day) => day.day === payload.day && day.missions.some((mission) => mission.id === payload.challengeId),
    );
  }
...
export async function POST(request: Request) {
  ...
  const limited = await enforceApiRateLimit(
    request,
    { scope: "upload.challenge", limit: 30, windowSeconds: 600 },
    user.id,
  );
