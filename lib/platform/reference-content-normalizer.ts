...
export const countryUsefulInfoSchema = z
  .array(
    z.object({
  ...
  .length(countryUsefulInfoCategories.length)
  .superRefine((items, context) => {
    for (const category of countryUsefulInfoCategories) {
      ...
      if (count !== 1)
        context.addIssue({
          code: "custom",
          message: `La categoria '${category}' deve comparire esattamente una volta`,
        });
    }
    ...
    if (!embassy?.url || !/^https:\/\/[^/]+\.esteri\.it(?:\/|$)/i.test(embassy.url)) {
      context.addIssue({
        code: "custom",
        message: "Il riferimento dell'Ambasciata deve usare un dominio ufficiale esteri.it",
      });
    }
    if (!embassy?.phone.trim()) {
      context.addIssue({
        code: "custom",
        message: "Il telefono dell'Ambasciata deve essere valorizzato nel campo dedicato",
      });
    }
    ...
    if (!emergency?.url || !emergency.phone.trim()) {
      context.addIssue({
        code: "custom",
        message: "I numeri di emergenza devono avere telefono e fonte ufficiale nei campi dedicati",
      });
    }
    const documents = items.find((item) => item.category === "Documenti e sicurezza");
    if (
      documents &&
      /(?:non (?:serve|e' richiesto|è richiesto) il visto)[\s\S]{0,500}(?:visto (?:obbligatorio|necessario))|(?:visto (?:obbligatorio|necessario))[\s\S]{0,500}(?:non (?:serve|e' richiesto|è richiesto) il visto)/i.test(
        documents.body,
      )
    ) {
      context.addIssue({
        code: "custom",
        message: "Le informazioni sui documenti contengono indicazioni contraddittorie sul visto",
      });
    }
  });
  ...
  .superRefine((items, context) => {
    ...
    for (const [language, phrases] of byLanguage) {
      if (["inglese", "english", "italiano", "italian"].includes(language)) {
        context.addIssue({
          code: "custom",
          message: `La lingua '${language}' non può essere usata come lingua locale`,
        });
      }
      ...
      if (actual.size !== expected.length || expected.some((translation) => !actual.has(translation))) {
        context.addIssue({
          code: "custom",
          message: `La lingua '${language}' deve tradurre le 12 frasi italiane richieste`,
        });
      }
    }
    if (byLanguage.size > 1) {
      const translations = [...byLanguage.values()].map(
        (phrases) => new Set(phrases.map((phrase) => phrase.translation.trim().toLocaleLowerCase("it"))),
      );
      if (
        translations.some((set) => set.size !== 12 || [...translations[0]].some((translation) => !set.has(translation)))
      ) {
...
const unsafeBingoPeople =
  /\b(donna|donne|uomo|uomini|persona|persone|bambino|bambina|bambini|ragazzo|ragazza|volto|volti|hijab|velo|fedeli|passanti)\b/i;
const unsafeBingoAccess =
  /\b(interno|interni|entrare|museo|negozio|workshop|laboratorio|lezione|pernottamento|noleggio|guida|comprare|acquistare)\b/i;
function bingoRequestsUnsafeSubject(copy: string) {
  const withoutSafetyExclusions = copy
    .replace(
      /\b(?:senza|escludi|evita)\s+(?:alcun[ao]?\s+)?(?:persona|persone|volto|volti|passante|passanti)(?:\s+(?:identificabile|identificabili|riconoscibile|riconoscibili))?/gi,
      "",
    )
    .replace(
      /\b(?:capo|abito)\s+tradizionale\s+esposto\s+senza\s+(?:persona|persone)\b/gi,
      "capo tradizionale esposto",
    );
  ...
  .superRefine((items, context) => {
    ...
    items.forEach((item, index) => {
      ...
      if (!item.description.trim().toLocaleLowerCase("it").startsWith("fotografa")) {
        context.addIssue({
          ...
          message: "La casella deve richiedere una prova fotografica",
        });
      }
      if (bingoRequestsUnsafeSubject(copy)) {
        context.addIssue({
          ...
          path: [index],
          message:
            "La casella non deve richiedere fotografie di persone identificabili, accessi, prenotazioni o acquisti",
        });
      }
    });
    for (const category of countryBingoCategories) {
      if (items.filter((item) => item.category === category).length !== 1) {
        context.addIssue({
          code: "custom",
          message: `La categoria bingo '${category}' deve comparire esattamente una volta`,
        });
      }
    }
  });
...
export const destinationReferenceSchema = z.object({
  quiz: z
        ...
        options: z
          ...
          .superRefine((options, context) => {
            ...
            if (new Set(normalized).size !== 4)
              context.addIssue({ code: "custom", message: "Le quattro opzioni devono essere diverse" });
          }),
    ...
    .max(15),
  missions: z
    .array(z.object({ title: z.string(), description: z.string(), photoValidation: photoValidationSchema.optional() }))
    .length(5),
  games: z
    .array(
      z
        ...
        .superRefine((game, context) => {
          ...
          if (game.type === "odd_one_out" && (game.options?.length !== 4 || game.correctIndex == null)) {
            context.addIssue({
              code: "custom",
              message: "Trova l'intruso deve contenere quattro opzioni e correctIndex",
            });
          }
          if (game.type === "odd_one_out" && (!game.commonRule || !game.intruderReason)) {
            context.addIssue({
              code: "custom",
              message: "Trova l'intruso deve motivare l'insieme comune e l'elemento estraneo",
            });
          }
        }),
    )
...
export function validateSiteReferenceContent(value: z.infer<typeof destinationReferenceSchema>, siteName: string) {
  ...
  if (new Set(questions).size !== questions.length)
    issues.push("Le domande del quiz del sito devono essere tutte diverse");
...
function validateNamedDestinationActivities(
  ...
  const groups = [
    ...
    ...value.photoContests.map((item, index) => ({
      path: `photoContests.${index}`,
  ...
  if (new Set(contestKeys).size !== contestKeys.length)
    issues.push("I contest fotografici devono essere tutti diversi");
...
export function normalizeReferenceContent(input: unknown, kind: "country" | "destination", destinationName = "") {
  ...
  if (kind === "country") {
    ...
    const normalizedBingo = Array.isArray(bingo)
      ? bingo.map((item, index) => {
          ...
          if (
            ...
            !(countryBingoCategories as readonly string[]).includes(category)
          )
            return item;
          ...
          return {
            ...cell,
            title,
            description: `Fotografa ${title.toLocaleLowerCase("it")} visibile da uno spazio pubblico o tra gli oggetti del viaggio.`,
  ...
  const oneBasedIndexes =
    indexes.length === quizItems.length &&
    indexes.length > 0 &&
    indexes.every((value) => value >= 1 && value <= 4) &&
    indexes.includes(4) &&
    !indexes.includes(0);
  ...
  const normalizedGames = ["photo_puzzle", "memory", "odd_one_out"]
    .map((type) => {
  ...
  const qualifyDestinationEntries = (value: unknown, field: string) =>
    Array.isArray(value)
  ...
  return {
    value: {
      ...
      photoContests: qualifyDestinationEntries(
        limitArray(source.photoContests, 2, "photoContests", changes),
        "photoContests",
      ),
    },
    changes,
