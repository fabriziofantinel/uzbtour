...
export async function readAgencyAgents(actorId: string, agencyId: string): Promise<AgencyAgent[]> {
  ...
  return rows.map((row) => ({
    id:String(row.id),name:String(row.name),username:String(row.username),email:String(row.email),
    phone:String(row.phone ?? ""),status:String(row.status) as AgencyAgent["status"],createdAt:String(row.created_at),
  }));
}

export async function createAgencyAgent(input: {
  actorId: string; agencyId: string; name: string; username: string; email: string; phone: string;
}) {
  const sql = getSql();
