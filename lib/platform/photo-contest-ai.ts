...
const score=z.object({entryId:z.string().uuid(),themeMatch:z.boolean(),themeReason:z.string().min(1).max(300),composition:z.number().int().min(0).max(100),technical:z.number().int().min(0).max(100),storytelling:z.number().int().min(0).max(100),originality:z.number().int().min(0).max(100),relevance:z.number().int().min(0).max(100),reason:z.string().min(1).max(1200)});
...
Valuta separatamente entrambe le foto. Prima stabilisci themeMatch: deve essere true soltanto se il soggetto visibile è realmente coerente con il luogo o tema indicato nel titolo e nelle istruzioni. La parola "libero" consente libertà stilistica, non di ignorare il soggetto indicato. Una foto fuori tema non può concorrere, indipendentemente dalla qualità, e deve avere themeMatch=false. Spiega la decisione in themeReason. Poi assegna: composizione 0-25, qualità tecnica 0-20, capacità narrativa 0-25, originalità 0-15, valorizzazione e aderenza al luogo/tema 0-15. Non identificare persone e non premiare attrezzatura, filigrane, testi o aspetto fisico. Per ogni foto scrivi una motivazione completa e concludila sempre con una frase intera, entro 1200 caratteri.`}];
    ...
    const evaluations=verdict.scores.map(item=>{const normalized={...item,eligible:item.themeMatch,composition:Math.min(25,item.composition),technical:Math.min(20,item.technical),storytelling:Math.min(25,item.storytelling),originality:Math.min(15,item.originality),relevance:item.themeMatch?Math.min(15,item.relevance):0};const rawTotal=normalized.composition+normalized.technical+normalized.storytelling+normalized.originality+normalized.relevance;return{...normalized,reason:item.themeMatch?item.reason:`Foto esclusa perché non coerente con il tema: ${item.themeReason}. ${item.reason}`,total:item.themeMatch?rawTotal:0};});
