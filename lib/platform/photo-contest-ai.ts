...
const score=z.object({entryId:z.string().uuid(),composition:z.number().int().min(0).max(100),technical:z.number().int().min(0).max(100),storytelling:z.number().int().min(0).max(100),originality:z.number().int().min(0).max(100),relevance:z.number().int().min(0).max(100),reason:z.string().min(1).max(1200)});
...
Valuta separatamente entrambe le foto. Griglia obbligatoria: composizione 0-25, qualità tecnica 0-20, capacità narrativa 0-25, originalità 0-15, valorizzazione e aderenza al luogo/tema 0-15. Per tema libero la rilevanza valuta la capacità di raccontare il viaggio. Non identificare persone e non premiare attrezzatura, filigrane, testi o aspetto fisico. Per ogni foto scrivi una motivazione completa e concludila sempre con una frase intera, entro 1200 caratteri.`}];
    ...
    const evaluations=verdict.scores.map(item=>{const normalized={...item,composition:Math.min(25,item.composition),technical:Math.min(20,item.technical),storytelling:Math.min(25,item.storytelling),originality:Math.min(15,item.originality),relevance:Math.min(15,item.relevance)};return{...normalized,total:normalized.composition+normalized.technical+normalized.storytelling+normalized.originality+normalized.relevance};});
