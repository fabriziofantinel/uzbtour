...
const score=z.object({themeMatch:z.boolean(),matchConfidence:z.number().min(0).max(1),themeReason:z.string().min(1).max(600),composition:z.number().int().min(0).max(100),technical:z.number().int().min(0).max(100),storytelling:z.number().int().min(0).max(100),originality:z.number().int().min(0).max(100),relevance:z.number().int().min(0).max(100),reason:z.string().max(1200).default("")});
...
export async function judgePhotoContest(input:{title:string;instructions:string;category:string;validationProfile?:unknown;photos:Array<{entryId:string;bytes:Uint8Array;contentType:string}>}){
  if(input.photos.length!==2)throw new Error("Servono esattamente due foto da valutare");
  const profileResult=photoValidationSchema.safeParse(input.validationProfile),profile=profileResult.success?profileResult.data:null;
...
Valuta questa foto. Prima stabilisci themeMatch e matchConfidence. Il titolo non è una prova visiva: confronta ciò che appare nella foto con target, visualDescription, requiredFeatures, acceptableVariations, rejectIf e confusableWith. themeMatch può essere true soltanto se il soggetto visibile è realmente coerente e non ricorre alcuna condizione rejectIf. Se gli elementi obbligatori non sono riconoscibili, se la foto mostra un soggetto confondibile diverso o se la confidence è inferiore a ${profile?.minimumConfidence??0.65}, usa themeMatch=false. La parola "libero" consente libertà stilistica, non di ignorare il soggetto indicato. Una foto fuori tema non può concorrere, indipendentemente dalla qualità. Spiega in themeReason quali prove visive hai trovato o quali elementi sono incompatibili. Poi assegna: composizione 0-25, qualità tecnica 0-20, capacità narrativa 0-25, originalità 0-15, valorizzazione e aderenza al luogo/tema 0-15. Non identificare persone e non premiare attrezzatura, filigrane, testi o aspetto fisico. Scrivi una motivazione completa e concludila sempre con una frase intera, entro 1200 caratteri.`;
  ...
  return Promise.all(input.photos.map(async(photo)=>{const response=await bedrock().send(new ConverseCommand({modelId,system:[{text:"Restituisci esclusivamente lo strumento richiesto. Applica la griglia in modo identico a ogni fotografia."}],messages:[{role:"user",content:[{text:prompt},{image:bedrockImage(photo.bytes,photo.contentType)}]}],toolConfig:{tools:[{toolSpec:{name:"emit_photo_contest_score",description:"Punteggio strutturato della fotografia",inputSchema:{json:z.toJSONSchema(verdictSchema,{target:"draft-7"})as unknown as DocumentType}}}],toolChoice:{tool:{name:"emit_photo_contest_score"}}},inferenceConfig:{maxTokens:650,temperature:0},requestMetadata:{application:"smf-travel",operation:"photo-contest-evaluation"}}));const item=toolInput(response.output?.message?.content).score;const threshold=profile?.minimumConfidence??0.65,eligible=item.themeMatch&&item.matchConfidence>=threshold,evaluationReason=item.reason||item.themeReason;const normalized={...item,entryId:photo.entryId,eligible,composition:Math.min(25,item.composition),technical:Math.min(20,item.technical),storytelling:Math.min(25,item.storytelling),originality:Math.min(15,item.originality),relevance:eligible?Math.min(15,item.relevance):0};const rawTotal=normalized.composition+normalized.technical+normalized.storytelling+normalized.originality+normalized.relevance;return{...normalized,reason:eligible?evaluationReason:`Foto esclusa perché non coerente con il tema: ${item.themeReason}. ${evaluationReason}`,total:eligible?rawTotal:0};}));
}
...
export async function processPhotoContestEvaluation(input:{jobId:string;agencyId:string;departureId:string;partyId:string;itemId:string;entryIds:string[]}){
  ...
  try{
    ...
    const itemPayload=rows[0].payload&&typeof rows[0].payload==="object"&&!Array.isArray(rows[0].payload)?rows[0].payload as Record<string,unknown>:{};
    const evaluations=await judgePhotoContest({title:String(rows[0].title),instructions:String(rows[0].instructions||itemPayload.description||""),category:String(rows[0].contest_category||"theme"),validationProfile:itemPayload.photoValidation,photos:rows.map((row,index)=>({entryId:String(row.id),bytes:images[index].bytes,contentType:images[index].contentType}))});
