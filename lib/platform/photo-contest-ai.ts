...
const score=z.object({themeMatch:z.boolean(),themeReason:z.string().min(1).max(300),composition:z.number().int().min(0).max(100),technical:z.number().int().min(0).max(100),storytelling:z.number().int().min(0).max(100),originality:z.number().int().min(0).max(100),relevance:z.number().int().min(0).max(100),reason:z.string().min(1).max(1200)});
...
export async function judgePhotoContest(input:{title:string;instructions:string;category:string;photos:Array<{entryId:string;bytes:Uint8Array;contentType:string}>}){
...
Valuta questa foto. Prima stabilisci themeMatch: deve essere true soltanto se il soggetto visibile è realmente coerente con il luogo o tema indicato nel titolo e nelle istruzioni. La parola "libero" consente libertà stilistica, non di ignorare il soggetto indicato. Una foto fuori tema non può concorrere, indipendentemente dalla qualità, e deve avere themeMatch=false. Spiega la decisione in themeReason. Poi assegna: composizione 0-25, qualità tecnica 0-20, capacità narrativa 0-25, originalità 0-15, valorizzazione e aderenza al luogo/tema 0-15. Non identificare persone e non premiare attrezzatura, filigrane, testi o aspetto fisico. Scrivi una motivazione completa e concludila sempre con una frase intera, entro 1200 caratteri.`;
  const modelId=process.env.AWS_BEDROCK_TEXT_MODEL?.trim();if(!modelId)throw new Error("AWS_BEDROCK_TEXT_MODEL non configurato");
  return Promise.all(input.photos.map(async(photo)=>{const response=await bedrock().send(new ConverseCommand({modelId,system:[{text:"Restituisci esclusivamente lo strumento richiesto. Applica la griglia in modo identico a ogni fotografia."}],messages:[{role:"user",content:[{text:prompt},{image:bedrockImage(photo.bytes,photo.contentType)}]}],toolConfig:{tools:[{toolSpec:{name:"emit_photo_contest_score",description:"Punteggio strutturato della fotografia",inputSchema:{json:z.toJSONSchema(verdictSchema,{target:"draft-7"})as unknown as DocumentType}}}],toolChoice:{tool:{name:"emit_photo_contest_score"}}},inferenceConfig:{maxTokens:550,temperature:0},requestMetadata:{application:"smf-travel",operation:"photo-contest-evaluation"}}));const item=toolInput(response.output?.message?.content).score;const normalized={...item,entryId:photo.entryId,eligible:item.themeMatch,composition:Math.min(25,item.composition),technical:Math.min(20,item.technical),storytelling:Math.min(25,item.storytelling),originality:Math.min(15,item.originality),relevance:item.themeMatch?Math.min(15,item.relevance):0};const rawTotal=normalized.composition+normalized.technical+normalized.storytelling+normalized.originality+normalized.relevance;return{...normalized,reason:item.themeMatch?item.reason:`Foto esclusa perché non coerente con il tema: ${item.themeReason}. ${item.reason}`,total:item.themeMatch?rawTotal:0};}));
}
...
export async function processPhotoContestEvaluation(input:{jobId:string;agencyId:string;departureId:string;partyId:string;itemId:string;entryIds:string[]}){
  ...
  try{
    ...
    const evaluations=await judgePhotoContest({title:String(rows[0].title),instructions:String(rows[0].instructions||((rows[0].payload as Record<string,unknown>)?.description)||""),category:String(rows[0].contest_category||"theme"),photos:rows.map((row,index)=>({entryId:String(row.id),bytes:images[index].bytes,contentType:images[index].contentType}))});
