...
import { addTravelerExpenseV3, deleteTravelerExpenseV3, readV3ExpenseRows } from "./v3-expenses";
import { readV3JourneyJournalRows } from "./v3-journey-journal";
import {
  addTravelerCashMovementV3,
  addTravelerRestaurantV3,
  deleteTravelerCashMovementV3,
  saveTravelerNoteV3,
} from "./v3-journey-mutations";
import { readV3ProgrammeFeedbackRows } from "./v3-programme-feedback";
import { readV3TravelCatalog } from "./v3-travel-catalog";
import { readV3Gamification } from "./v3-gamification";
import {
  readV3TravelerJourneys,
  readV3TravelerDestinationProfile,
...
export async function getTravelerExperience(userId: string, requestedDepartureId?: string) {
  ...
  const missingCities = [
    ...new Map(
      cities.filter((city) => city.latitude == null || city.longitude == null).map((city) => [String(city.id), city]),
    ).values(),
  ];
  if (missingCities.length > 0) {
    ...
    recovered.forEach((result, index) => {
      if (result.status === "rejected") {
        console.warn(
          `Coordinate non recuperate per ${String(missingCities[index].name)}`,
  ...
  return {
    journey: {
      ...
      destinationTimeZone: stringValue(destinationProfile?.time_zone) || String(selected.timezone),
      destinationCurrency: stringValue(destinationProfile?.currency_code),
      ...
      travelers: activeTravelerRows.map((row) => ({
        ...
        participatesInTripGames: Boolean(row.participates_in_trip_games),
    ...
    days: activeDayRows.map((row) => {
      ...
      return {
        ...
        documents: activeDayDocumentRows
          .filter((document) => String(document.day_id) === id)
          .map((document) => ({
        ...
        items: items
          .filter((item) => String(item.trip_day_id) === id)
          .map((item) => ({
            id: String(item.id),
            ...
            tickets: tickets
              .filter((ticket) => String(ticket.itinerary_item_id) === String(item.id))
              .map((ticket) => ({
                id: String(ticket.id),
                title: String(ticket.title),
                contentType: String(ticket.content_type),
        ...
        cities: cities
          .filter((city) => String(city.trip_day_id) === id)
          .map((city) => ({
            id: String(city.id),
            name: String(city.name),
            country: String(city.country),
            googleUrl: String(city.google_url),
            latitude: city.latitude == null ? null : Number(city.latitude),
            longitude: city.longitude == null ? null : Number(city.longitude),
          })),
        sites: sites
          .filter((site) => String(site.trip_day_id) === id)
          .map((site) => ({
            id: String(site.id),
            name: String(site.name),
            city: String(site.city),
            googleUrl: String(site.google_url),
            officialUrl: stringValue(site.official_url),
            latitude: site.latitude == null ? null : Number(site.latitude),
            longitude: site.longitude == null ? null : Number(site.longitude),
          })),
        hotels: hotels
          .filter((hotel) => String(hotel.trip_day_id) === id)
          .map((hotel) => ({
            id: String(hotel.id),
            name: String(hotel.name),
            city: String(hotel.city),
            googleUrl: String(hotel.google_url),
            websiteUrl: stringValue(hotel.website_url),
    ...
    challenges: activeChallengeRows.map((row) => ({
      ...
      title: String(row.title),
      content: withoutAnswerKeys(row.content),
    })),
    expenses: activeExpenseRows.map((row) => ({
      ...
      shares: Array.isArray(row.shares)
        ? row.shares.map((share) => {
            ...
            return {
              travelerId: String(value.travelerId),
              travelerName: String(value.travelerName),
              amount: Number(value.amount),
              baseAmount: Number(value.baseAmount),
            };
          })
        : [],
    })),
    ...
    cashMovements: activeCashRows.map((row) => ({
      ...
      euroAmount: row.euro_amount == null ? null : Number(row.euro_amount),
    ...
    photos: activePhotoRows.map((row) => ({
      ...
      addedBy: stringValue(row.added_by) || "Viaggiatore",
      createdAt: String(row.created_at),
    ...
    challengeResults: activeResultRows.map((row) => ({
      ...
      travelerName: String(row.display_name),
      dayId: row.trip_day_id ? String(row.trip_day_id) : null,
      contentId: String(row.generated_content_id),
      type: String(row.activity_type),
      score: Number(row.score),
      maxScore: row.max_score == null ? null : Number(row.max_score),
      status: String(row.status),
      result: withoutAnswerKeys(row.result),
    ...
    contestEntries: activeContestRows.map((row) => ({
      ...
      travelerName: String(row.display_name),
      contentId: String(row.generated_content_id),
      mediaId: String(row.media_asset_id),
