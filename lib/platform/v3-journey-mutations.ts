...
function scope(actorId: string, agencyId: string, departureId: string, partyId: string, dayId: string) {
  return { actorId, agencyId, departureId, partyId, dayId };
}
...
export async function saveTravelerNoteV3(input: {
  ...
  const sql = getSql(),
    s = scope(input.userId, input.agencyId, input.departureId, input.partyId, input.dayId),
