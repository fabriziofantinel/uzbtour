...
export async function saveTravelerNoteV3(input: {
  agencyId: string;
  actorUserId: string;
  departureId: string;
  partyId: string;
  ...
  const sql = getSql(),
    s = scope(input.actorUserId, input.agencyId, input.departureId, input.partyId, input.dayId),
    operationId = randomUUID();
  const [, rows] = await sql.transaction((txn) => [
    txn`SELECT set_config('app.agency_id',${s.agencyId},true)`,
    txn`
  WITH actor AS (SELECT traveler.id FROM travel.traveler_profiles traveler JOIN travel.party_memberships membership
   ...
   AND membership.departure_id=${s.departureId} AND membership.party_id=${s.partyId} AND membership.status='active'
    WHERE traveler.agency_id=${s.agencyId} AND traveler.user_id=${s.actorUserId}::uuid),
...
export async function addTravelerRestaurantV3(input: {
  agencyId: string;
  actorUserId: string;
  departureId: string;
  partyId: string;
...
export async function addTravelerCashMovementV3(input: {
  agencyId: string;
  actorUserId: string;
  departureId: string;
  partyId: string;
...
export async function deleteTravelerCashMovementV3(input: {
  agencyId: string;
  actorUserId: string;
  departureId: string;
  partyId: string;
  ...
  WHERE movement.id=${input.movementId} AND movement.agency_id=${input.agencyId} AND movement.departure_id=${input.departureId} AND movement.party_id=${input.partyId}
    AND traveler.agency_id=movement.agency_id AND traveler.user_id=${input.actorUserId}::uuid
