...
export async function createV3PlatformAgencyWithOwner(input: Record<string, string> & {
  actorId: string;
  tokenHash: string;
  expiresAt: string;
}) {
  ...
  let rows;
  try{rows = await sql`SELECT * FROM app.create_platform_agency_with_owner(
    ${input.actorId},${payload}::jsonb,${input.tokenHash},${input.expiresAt}::timestamptz)`;}
  catch(error){if(isUniqueViolation(error))throw new PlatformRequestError("Nome agenzia o username responsabile già presente");throw error;}
...
export async function updateV3PlatformAgencyStatus(input:{actorId:string;agencyId:string;status:"trial"|"active"|"suspended"}){
  const sql=getSql();const rows=await sql`SELECT app.update_platform_agency_status(
...
export async function replaceV3PlatformAgencyOwner(input:{actorId:string;agencyId:string;name:string;initials:string;username:string;email:string;phone:string;tokenHash:string;expiresAt:string}){
