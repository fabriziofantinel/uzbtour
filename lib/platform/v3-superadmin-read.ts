...
export async function readV3AgencyRegistry(actorId: string) {
  ...
  return agencyIds.map((id) => {
    ...
    return {
      id, slug: String(row.slug), name: String(row.agency_name), status: String(row.agency_status),
      legalName: String(row.legal_name), vatNumber: String(row.vat_number), taxCode: String(row.tax_code),
      registeredAddress: String(row.registered_address), registeredCity: String(row.registered_city),
      registeredPostalCode: String(row.registered_postal_code), registeredProvince: String(row.registered_province),
      registeredCountry: String(row.registered_country), pec: String(row.pec), sdiCode: String(row.sdi_code),
      phone: String(row.agency_phone), email: String(row.agency_email), website: String(row.website),
      referenceName: String(row.reference_name), referenceEmail: String(row.reference_email),
