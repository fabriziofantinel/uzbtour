...
export async function readCountryProfilesForReview(actorId: string) {
  ...
  return rows.map((row) => ({
    agencyId: String(row.agency_id), countryId: String(row.country_id), countryName: String(row.country_name), iso2: String(row.iso2 || ""),
...
export async function reviewCountryProfile(actorId: string, agencyId: string, countryId: string, approve: boolean) {
  const sql = getSql();
  const rows = await sql`SELECT app.review_country_profile_v3(${actorId},${agencyId}::uuid,${countryId}::uuid,${approve}) AS updated`;
