...
export async function downloadTripForOffline(
  ...
  return new Promise<OfflinePackageResult>((resolve, reject) => {
    ...
    channel.port1.onmessage = (event) => {
      ...
      if (data.type === "OFFLINE_PROGRESS") onProgress(data.done, data.total);
      if (data.type === "OFFLINE_READY") resolve({ skipped: Number(data.skipped) || 0 });
      if (data.type === "OFFLINE_FAILED") reject(new Error(data.message || "Download offline non riuscito"));
    };
