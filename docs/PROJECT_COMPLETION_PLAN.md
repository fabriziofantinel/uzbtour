...
Checklist predisposta in `docs/operations/GO_LIVE_CHECKLIST.md`. La capacità Vercel è stata bonificata l'8 settembre 2026 riducendo l'inventario da 213 a 24 deployment, mantenendo 20 release di produzione utilizzabili per rollback e 4 deployment protetti da alias; retention a 7 giorni e `ignoreCommand` limitano la crescita futura. Due restore drill Neon consecutivi sono stati completati l'8 settembre 2026 su punti a -5 e -10 minuti, con validazione integra e tempi operativi di 15 e 14 secondi. Il rollback Vercel alla release precedente e il ripristino della corrente sono stati dimostrati con HTTP 200 e compatibilità dello schema verificata. SQS, Lambda, CloudWatch e Cognito risultano operativi; anche il redrive DLQ è stato provato con un job innocuo e ha permesso di individuare e correggere in produzione la variabile `WORKLOAD` del worker import, senza eseguire Bedrock. Cloudflare R2 è stato verificato dalla console senza estrarre credenziali: bucket privato UE, CORS produzione, Bucket Lock a 30 giorni e recupero integro di un campione DOCX. La richiesta di uscita dalla sandbox SES, corredata con il dettaglio operativo richiesto, è stata respinta a livello SES; il caso Support resta aperto in attesa della risposta AWS. Escludendo gli UAT richiesti dall'operatore, non restano attività tecniche eseguibili: il solo vincolo esterno è lo sblocco SES da parte di AWS. Lo script storico dei dati demo richiede revisione prima di qualsiasi esecuzione perché usa ancora strutture legacy e identificativi fissi.
