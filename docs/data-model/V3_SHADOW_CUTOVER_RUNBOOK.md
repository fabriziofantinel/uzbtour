...
- Il ruolo runtime `smf_app` non ha ancora accesso agli schemi v3.
...
1. Backfill di contenuti, media, documenti e dati di esperienza (`content`, `ops`,
   `journey`, `privacy`).
2. Concessione minima al ruolo runtime e impostazione obbligatoria del contesto
   tenant per transazione.
