...
export default function ImpersonationBanner() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me", { cache: "no-store" })
      ...
      .then((payload) => {
        if (active) setIdentity(payload?.user ?? null);
      })
      .catch(() => undefined);
    return () => {
      active = false;
  ...
  return (
    <aside className="impersonationBanner" role="status">
      <span>
        <ShieldCheck size={17} />
        <b>{identity.impersonation.actorName}</b> sta operando come <strong>{identity.name}</strong>
      </span>
      <form action="/api/auth/impersonation/stop" method="post">
        <button type="submit">
          <LogIn size={16} /> {identity.impersonation.actorIsSuperAdmin ? "Torna al superadmin" : "Torna all’agenzia"}
        </button>
      </form>
    </aside>
  );
