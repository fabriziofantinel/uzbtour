...
export default function PlatformTripChallenges({ experience, userName, isAdmin, onResultsChange }: {
  ...
  return <section className="challengesPage">
    ...
    {tab === "foto" && <section className="contestHub"><header className="contestHubHero"><span><Camera/></span><div><small>DUE CONTEST · TRE FOTO A TESTA</small><h3>Contest fotografici</h3><p>Sempre sbloccati: partecipa alla gara a tema libero e alla sfida proposta.</p></div></header><DayPicker days={experience.days} activeDayId={day.id} onDay={setActiveDayId} count={(entry) => experience.challenges.filter((item) => item.type === "photo_contest" && item.dayId === entry.id).length}/><div className="dualContestDayHeading"><div><small>GIORNO {day.number} · {dateLabel(day.date)}</small><h3>{day.city}</h3></div><span>Massimo 3 foto per partecipante e per contest</span></div>{contests.length === 0 ? <ChallengeEmpty icon={<Camera/>} title="Contest in preparazione" copy="I due contest fotografici appariranno qui appena saranno pubblicati."/> : <div className="dualContestGrid">{contests.map((contest, index) => { const entries = contestEntries.filter((entry) => entry.contentId === contest.id); const ownEntries = entries.filter((entry) => entry.travelerName === userName); return <article className={`dualContestCard ${index === 0 ? "free" : "theme"}`} key={contest.id}><header><span>{index === 0 ? <Camera/> : <Sparkles/>}</span><div><small>CONTEST {index + 1} · {index === 0 ? "TEMA LIBERO" : "TEMA DEL GIORNO"}</small><h3>{contest.title}</h3><p>{text(contest.content, "description")}</p></div><b>{entries.length} foto</b></header><div className="contestEntryGrid">{entries.map((entry) => entry.contentUrl && <figure key={entry.id}><div><Image src={entry.contentUrl} alt={`Foto contest caricata da ${entry.travelerName}`} fill sizes="180px" unoptimized/></div><figcaption>{entry.travelerName}</figcaption></figure>)}</div><div className="contestUploadBar"><div><strong>Le tue foto: {ownEntries.length}/3</strong><small>{ownEntries.length < 3 ? "Puoi aggiungere altre foto" : "Limite raggiunto"}</small></div><PhotoPicker busy={busy === `${contest.id}-photo`} disabled={ownEntries.length >= 3} label="Carica foto" onFile={(file) => void uploadEvidence(contest, day, file)}/></div><div className="contestJudgeBar">{isAdmin ? <button type="button" disabled={entries.length === 0}><Sparkles/> Avvia valutazione</button> : <p>La valutazione può essere avviata dall’amministratore.</p>}</div></article>; })}</div>}</section>}
...
function DayPicker({ days, activeDayId, onDay, count }: { days: Day[]; activeDayId: string; onDay: (id: string) => void; count: (day: Day) => number }) {
  return <div className="challengeDayPicker" aria-label="Scegli la giornata">{days.map((day) => <button type="button" className={day.id === activeDayId ? "active" : ""} aria-pressed={day.id === activeDayId} aria-label={`Giorno ${day.number}, ${dateLabel(day.date)}, ${count(day)} sfide`} onClick={() => onDay(day.id)} key={day.id}><small>GIORNO {day.number}</small><strong>{dateLabel(day.date)}</strong><b>{count(day)} sfide</b></button>)}</div>;
}
