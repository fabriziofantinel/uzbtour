...
export default function PlatformTripChallenges({ experience, userName, isAdmin, onResultsChange }: {
  ...
  return <section className="challengesPage">
    ...
    <nav className={`challengeTabs${isAdmin ? " admin" : ""}`} aria-label="Tipi di sfida">
      <button type="button" className={tab === "missioni" ? "active" : ""} aria-current={tab === "missioni" ? "page" : undefined} onClick={() => setTab("missioni")}><Compass/>Missioni</button>
      <button type="button" className={tab === "bingo" ? "active" : ""} aria-current={tab === "bingo" ? "page" : undefined} onClick={() => setTab("bingo")}><Grid3X3/>Bingo</button>
      <button type="button" className={tab === "foto" ? "active" : ""} aria-current={tab === "foto" ? "page" : undefined} onClick={() => setTab("foto")}><Camera/>Foto</button>
      <button type="button" className={tab === "quiz" ? "active" : ""} aria-current={tab === "quiz" ? "page" : undefined} onClick={() => setTab("quiz")}><Brain/>Quiz</button>
      <button type="button" className={tab === "giochi" ? "active" : ""} aria-current={tab === "giochi" ? "page" : undefined} onClick={() => setTab("giochi")}><Gamepad2/>Giochi</button>
      <button type="button" className={tab === "profilo" ? "active" : ""} aria-current={tab === "profilo" ? "page" : undefined} onClick={() => setTab("profilo")}><Award/>Profilo</button>
      <button type="button" className={tab === "classifica" ? "active" : ""} aria-current={tab === "classifica" ? "page" : undefined} onClick={() => setTab("classifica")}><Trophy/>Classifica</button>
      {isAdmin && <button type="button" className={tab === "valida" ? "active" : ""} aria-current={tab === "valida" ? "page" : undefined} onClick={() => setTab("valida")}><CheckCircle2/>Valida <b>{challengeResults.filter((result) => result.status === "submitted" && ["mission", "bingo"].includes(result.type)).length}</b></button>}
    </nav>
