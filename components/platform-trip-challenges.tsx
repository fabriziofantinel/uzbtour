...
export default function PlatformTripChallenges({ experience, userName, isAdmin, onResultsChange }: {
    ...
    try {
      ...
      if (challenge.type === "photo_contest") setContestEntries((current) => {
        const entry={id:linked.id!,travelerId:"current",travelerName:userName,contentId:challenge.id,
          mediaId:registered.photo!.mediaId,slot:linked.slot||1,status:"draft",score:null,reason:"",criteria:{},isWinner:false,
  ...
  return <section className="challengesPage">
    ...
    {tab === "bingo" && <section className="bingoBoard"><div className="bingoHeading"><div><span>SEMPRE SBLOCCATO · CARTELLA 3 × 9</span><h3>Tombola fotografica {experience.journey.destinationCountry}</h3><p>Carica una foto: viene valutata automaticamente. Se è respinta puoi fare un solo secondo tentativo.</p></div><strong>{bingo.filter((item) => approved.has(item.id)).length}/15<small>caselle validate</small></strong></div><div className="bingoAwards">{bingoRows.map((row, index) => { const milestone = bingoMilestone(row.filter((item) => approved.has(item.id)).length); return <span className={milestone ? "won" : ""} key={index}>Riga {index + 1}<b>{milestone ? `${milestone.label} · ${milestone.points} pt` : "In gioco"}</b></span>; })}<span className={bingo.length === 15 && bingo.every((item) => approved.has(item.id)) ? "won tombola" : ""}>Cartella completa<b>Tombola · 50 pt</b></span></div><div className="tombolaScroller"><div className="tombolaCard">{bingoCells.map(({ rowIndex, columnIndex, item }) => {
      if (!item) return <span className="tombolaBlank" key={`${rowIndex}-${columnIndex}`}/>;
      const latestEvidence = challengeResults.find((result) => result.travelerName === userName && result.contentId === item.id && Boolean(result.evidenceUrl));
...
function ContestPhotoDetails({ entry, title, isOwn, wonOnTie }: {
  ...
  return <>
    ...
    {open && createPortal(<div className="contestPhotoViewer" role="dialog" aria-modal="true" aria-label={`Valutazione foto di ${entry.travelerName}`} onClick={() => setOpen(false)}>
      ...
      <aside className="contestPhotoViewerCopy" onClick={(event) => event.stopPropagation()}><small>{entry.isWinner ? "FOTO VINCITRICE" : isOwn ? "LA MIA FOTO" : "FOTO DEL CONTEST"}</small><h3>{title}</h3><span>{entry.travelerName}</span><strong>{scoreLabel}</strong>{criteria.some(([, value]) => value != null) && <dl className="contestPhotoCriteria">{criteria.map(([label,value,maximum]) => value == null ? null : <div key={label}><dt>{label}</dt><dd><span style={{width:`${Math.min(100,Number(value)/maximum*100)}%`}}/><b>{value}/{maximum}</b></dd></div>)}</dl>}<p>{entry.reason || "Il giudizio AI non è ancora disponibile."}</p>{wonOnTie && <div className="contestPhotoViewerTie">Parità di punteggio: questa foto ha vinto perché è stata caricata prima.</div>}</aside>
    </div>, document.body)}
  </>;
}
