...
const source=await readFile(new URL("../database/migrations/118_v3_operational_messages_analytics_policy.sql",import.meta.url),"utf8");
...
try{await client.connect();await client.query("BEGIN");open=true;await client.query("SET LOCAL lock_timeout='5s'");await client.query(source);
 const gate=(await client.query(`SELECT EXISTS(SELECT 1 FROM pg_policies WHERE schemaname='journey'
  AND tablename='operational_messages' AND policyname='operational_messages_tenant_read_policy') policy_ok,
  has_table_privilege('smf_app','journey.operational_messages','SELECT') select_ok`)).rows[0];
