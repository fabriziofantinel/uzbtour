...
const name = "068_v3_agency_agent_removal",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.remove_agency_agent_v3(text,uuid,text)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.39.0-agency-agent-removal", createHash("sha256").update(source).digest("hex")],
    );
