...
const name="083_v3_traveler_trip_competition";
const model="3.53.0-traveler-trip-competition";
...
try{
  ...
  const gate=(await client.query(`SELECT
    has_function_privilege('smf_app','app.update_journey_traveler_competition(text,uuid,uuid,uuid,uuid,boolean)','EXECUTE') mutation_ok,
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='travel' AND table_name='party_memberships' AND column_name='participates_in_trip_games') traveler_scope_ok`)).rows[0];
