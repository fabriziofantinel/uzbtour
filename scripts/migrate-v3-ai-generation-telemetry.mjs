...
const name = "139_v3_ai_generation_telemetry";
const model = "3.84.0-ai-generation-telemetry";
...
try {
  ...
  const gate = (
    ...
    to_regclass('ops.ai_model_prices') IS NOT NULL prices,
    to_regclass('ops.generation_runs_platform_job_idx') IS NOT NULL job_index,
    has_function_privilege('smf_app','app.record_ai_generation_v3(uuid,uuid,text,text,text,text,text,text,integer,integer,integer,text)','EXECUTE') recorder`)
  ).rows[0];
