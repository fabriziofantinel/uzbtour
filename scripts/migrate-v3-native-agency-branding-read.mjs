...
const name = "156_v3_native_agency_branding_read";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-native-agency-branding-read',0))");
  await client.query(source);
  const gate = (
    await client.query(`SELECT
    has_function_privilege('smf_app','app.read_agency_branding_v3(uuid)','EXECUTE') native_ready,
    NOT has_function_privilege('smf_app','app.read_agency_branding_v3(text)','EXECUTE') legacy_revoked`)
  ).rows[0];
  ...
  if (apply) {
    ...
    ON CONFLICT(version) DO UPDATE SET applied_at=clock_timestamp(),checksum_sha256=EXCLUDED.checksum_sha256`,
      ["3.101.0-native-agency-branding-read", checksum],
    );
