...
const name="058_v3_impersonation_contract_and_tenant_status",model="3.29.0-impersonation-contract-and-tenant-status";
...
try{
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-impersonation-contract',0))");
  await client.query(source);
  const gate=(await client.query(`SELECT
    has_function_privilege('smf_app','app.start_legacy_impersonation(text,text,text,timestamptz,text)','EXECUTE') runtime_start,
    has_function_privilege('smf_app','app.resolve_legacy_impersonation(text,text)','EXECUTE') runtime_resolve`)).rows[0];
  if(!gate.runtime_start||!gate.runtime_resolve)throw new Error("Contratto impersonazione non eseguibile dal runtime");
  ...
  if(apply){await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms)
    ...
    [model,checksum,executionMs]);await client.query("COMMIT");}
