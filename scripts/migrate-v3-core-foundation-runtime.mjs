...
try {
  ...
  const gates = (await client.query(`SELECT
    ...
    NOT has_table_privilege('smf_app','ops.legacy_id_map','SELECT') maps_private`)).rows[0];
  ...
  if (apply) {
    for (const migration of migrations) {
      const previous = await client.query("SELECT checksum_sha256 FROM ops.schema_migrations WHERE version=$1",[migration.model]);
      if (previous.rowCount && previous.rows[0].checksum_sha256!==migration.checksum) throw new Error(`Checksum differente per ${migration.model}`);
      await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms) VALUES($1,$2,$3)
        ON CONFLICT(version) DO UPDATE SET applied_at=clock_timestamp(),execution_ms=EXCLUDED.execution_ms`,[migration.model,migration.checksum,executionMs]);
    }
    await client.query("COMMIT");
  } else await client.query("ROLLBACK");
  open=false;
  console.log(JSON.stringify({status:apply?"applied":"dry_run_passed",role:capability.role_name,executionMs,gates,
    migrations:migrations.map(({name,model,checksum})=>({name,model,checksum}))},null,2));
