...
const url=process.env.DATABASE_URL;const ownerUrl=process.env.DATABASE_MIGRATION_URL??process.env.DATABASE_URL_UNPOOLED;if(!url)throw new Error("DATABASE_URL runtime non configurata");
const client=new Client(url);
const owner=ownerUrl?new Client(ownerUrl):null;
try{await client.connect();if(!owner)throw new Error("Connessione owner necessaria per preparare la fixture viaggio");await owner.connect();const role=(await client.query("SELECT current_user role_name")).rows[0]?.role_name;
 const denied=(await client.query("SELECT * FROM app.read_journey_management($1,$2)",[randomUUID(),randomUUID()])).rowCount===0;
 const candidate=(await owner.query(`SELECT actor.id actor_id,departure.id departure_id
