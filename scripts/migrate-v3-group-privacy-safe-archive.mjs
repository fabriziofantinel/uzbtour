...
const source=await readFile(new URL("../database/migrations/117_v3_group_privacy_safe_archive.sql",import.meta.url),"utf8");
...
try{await client.connect();await client.query("BEGIN");open=true;await client.query("SET LOCAL lock_timeout='5s'");await client.query(source);
 const gate=(await client.query(`SELECT has_function_privilege('smf_app','app.delete_empty_journey_party_v3(text,uuid,uuid,uuid)','EXECUTE') function_ok,
 pg_get_functiondef('app.delete_empty_journey_party_v3(text,uuid,uuid,uuid)'::regprocedure) LIKE '%privacy_safe_group_deletion%' archive_ok`)).rows[0];
 ...
 VALUES('3.67.0-group-privacy-safe-archive',$1,0) ON CONFLICT(version) DO UPDATE SET applied_at=clock_timestamp(),checksum_sha256=EXCLUDED.checksum_sha256`,[checksum]);await client.query("COMMIT");}
