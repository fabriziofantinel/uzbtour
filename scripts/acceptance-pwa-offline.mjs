...
class MemoryCache {
  ...
  key(request) {
    return typeof request === "string" ? new URL(request, "https://smf.test").href : request.url;
  }
  async match(request) {
  ...
  async addAll(urls) {
    for (const url of urls) {
      const request = new Request(new URL(url, "https://smf.test"));
      await this.put(request, await this.fetcher(request));
    }
  }
}
...
const responses = new Map([
  ...
  [
    ...
    new Response(JSON.stringify({ partyId: "party-alpha", days: [1, 2] }), {
      headers: { "Content-Type": "application/json" },
    }),
  ],
  [
    "/api/traveler/documents/doc-alpha/content",
    new Response("VOUCHER ALPHA", { headers: { "Content-Type": "application/pdf" } }),
  ],
]);
...
assert.equal(messages.at(-1)?.skipped, 0);
...
assert.equal(
  await (await offlineFetch("/api/traveler/trip-data?partenza=departure-alpha")).json().then((value) => value.partyId),
  "party-alpha",
);
...
listeners.get("fetch")({
  ...
  respondWith(value) {
    privateNavigationResponse = Promise.resolve(value);
  },
});
assert.equal(
  privateNavigationResponse,
  undefined,
  "Il back-office autenticato non deve essere servito dalla cache PWA",
);
...
console.log(
  JSON.stringify({
    status: "passed",
    cachedResources: 3,
    offlineProgramme: true,
    offlineTripData: true,
    offlineDocument: true,
    privateBackOfficeExcluded: true,
