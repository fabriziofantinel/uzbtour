...
const name = "154_v3_native_agency_agents_read";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-native-agency-agents-read',0))");
  await client.query(source);
  const gate = (
    await client.query(`SELECT
  has_function_privilege('smf_app','app.read_agency_agents_v3(uuid,uuid)','EXECUTE') native_ready,
  NOT has_function_privilege('smf_app','app.read_agency_agents_v3(text,uuid)','EXECUTE') legacy_revoked`)
  ).rows[0];
 ...
 ON CONFLICT(version) DO UPDATE SET applied_at=clock_timestamp(),checksum_sha256=EXCLUDED.checksum_sha256`,
      ["3.99.0-native-agency-agents-read", checksum],
    );
