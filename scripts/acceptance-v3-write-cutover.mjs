...
try {
  ...
  `,
      [scope.agency_id, scope.departure_id, scope.day_id],
    )
  ).rows;
  const stays = (
    await owner.query(
  ...
  const claimed = (
    await runtime.query(
      `SELECT app.claim_platform_job_v3($1,$2,
    'travel-reference.enrich') AS ok`,
      [job.id, scope.agency_id],
    )
  ).rows[0]?.ok;
  ...
  await runtime.query(
    `SELECT * FROM app.replace_trip_experience_v3(
