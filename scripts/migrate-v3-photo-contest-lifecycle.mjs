...
const name = "098_v3_photo_contest_lifecycle",
  model = "3.65.0-photo-contest-lifecycle",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.upsert_photo_contest_draft_v3(text,uuid,uuid,uuid,uuid,uuid,uuid,uuid,smallint)','EXECUTE') draft_ok,has_function_privilege('smf_app','app.confirm_photo_contest_v3(text,uuid,uuid,uuid,uuid)','EXECUTE') confirm_ok`,
    )
  ).rows[0];
