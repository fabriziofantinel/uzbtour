...
const name="060_v3_agency_owner_management",model="3.31.0-agency-owner-management";
...
const checksum=createHash("sha256").update(source).digest("hex"),client=new Client(url);let open=false;let gates;
try{
  ...
  gates=(await client.query(`SELECT
    ...
    has_function_privilege('smf_app','app.replace_platform_agency_owner(text,uuid,text,text,text,text,text,text,timestamptz)','EXECUTE') owner_replace`)).rows[0];
  if(!gates.username_check||!gates.contact_update||!gates.owner_replace)throw new Error("Contratti runtime owner incompleti");
  ...
  open=false;console.log(JSON.stringify({status:apply?"applied":"dry_run_passed",gates,executionMs},null,2));
