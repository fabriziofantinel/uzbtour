...
const source=await readFile(new URL("../database/migrations/120_v3_branded_web_push_subscriptions.sql",import.meta.url),"utf8");
...
try{await client.connect();await client.query("BEGIN");open=true;await client.query("SET LOCAL lock_timeout='5s'");await client.query(source);
 const gate=(await client.query(`SELECT has_function_privilege('smf_app','app.list_departure_branded_web_push_subscriptions_v3(uuid)','EXECUTE') departure_ok,
 has_function_privilege('smf_app','app.list_party_branded_web_push_subscriptions_v3(uuid,uuid)','EXECUTE') party_ok`)).rows[0];
 if(!gate||Object.values(gate).some((value)=>value!==true))throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
 if(apply){await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms) VALUES('3.70.0-branded-web-push',$1,0)
