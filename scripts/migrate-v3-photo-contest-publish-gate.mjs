...
const name = "111_v3_photo_contest_publish_gate",
  model = "3.37.0-photo-contest-publish-gate",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT pg_get_functiondef('app.validate_template_version_publish(uuid,uuid)'::regprocedure) LIKE '%max_entries = 2%' contest_limit_two`,
    )
  ).rows[0];
  if (!gate?.contest_limit_two) throw new Error(`Gate incompleto: ${JSON.stringify(gate)}`);
  if (apply) {
    await client.query(
