...
const name = "114_v3_scheduled_push_claim_contract",
  model = "3.40.0-scheduled-push-claim-contract",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.claim_due_push_deliveries_v3()','EXECUTE') runtime_execute,pg_get_functiondef('app.claim_due_push_deliveries_v3()'::regprocedure) LIKE '%ON CONFLICT ON CONSTRAINT push_delivery_runs_departure_id_event_key_key%' explicit_conflict`,
    )
  ).rows[0];
