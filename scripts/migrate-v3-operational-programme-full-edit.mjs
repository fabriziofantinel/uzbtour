...
try{
  ...
  const gate=(await client.query(`SELECT
    ...
    EXISTS(SELECT 1 FROM pg_constraint WHERE conrelid='travel.departure_itinerary_items'::regclass AND conname='departure_items_timed_type_check') AS timed_types_ready`)).rows[0];
  if(!gate?.can_update||!gate?.timed_types_ready)throw new Error("Contratto di modifica programma non disponibile");
  if(apply){
    await client.query("INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms) VALUES($1,$2,0) ON CONFLICT(version) DO UPDATE SET applied_at=clock_timestamp()",["3.48.0-operational-programme-full-edit",createHash("sha256").update(source).digest("hex")]);
