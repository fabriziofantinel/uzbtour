...
const source=await readFile(new URL("../database/migrations/121_v3_memory_privacy_audit.sql",import.meta.url),"utf8");
...
try{await client.connect();await client.query("BEGIN");open=true;await client.query("SET LOCAL lock_timeout='5s'");await client.query(source);
 const gate=(await client.query(`SELECT
   ...
   NOT has_table_privilege('smf_app','ops.audit_events','INSERT') audit_table_private`)).rows[0];
 if(!gate||Object.values(gate).some((value)=>value!==true))throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
 if(apply){await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms) VALUES('3.71.0-memory-privacy-audit',$1,0)
