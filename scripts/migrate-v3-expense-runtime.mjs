...
const migrationVersion = "019_v3_expense_runtime_access";
const modelVersion = "3.3.1-runtime-expenses";
...
const migrationSql = await readFile(
  new URL("../database/migrations/019_v3_expense_runtime_access.sql", import.meta.url),
  "utf8",
);
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-runtime-expenses', 0))");
  await client.query(migrationSql);

  const grants = (await client.query(`
    SELECT
      has_schema_privilege('smf_app', 'journey', 'USAGE') AS journey_usage,
      has_table_privilege('smf_app', 'journey.expenses', 'SELECT') AS can_select,
      has_table_privilege('smf_app', 'journey.expenses', 'INSERT') AS can_insert,
      has_table_privilege('smf_app', 'journey.expenses', 'UPDATE') AS can_update,
      has_table_privilege('smf_app', 'journey.expenses', 'DELETE') AS can_delete,
