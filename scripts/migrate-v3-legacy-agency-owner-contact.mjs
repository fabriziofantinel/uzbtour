...
const name = "062_v3_legacy_agency_owner_contact",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.update_platform_agency_owner_contact(text,uuid,text,text)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.33.0-legacy-agency-owner-contact", createHash("sha256").update(source).digest("hex")],
    );
