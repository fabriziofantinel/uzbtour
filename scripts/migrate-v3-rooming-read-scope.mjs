...
const name = "201_v3_rooming_read_scope";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-rooming-read-scope',0))");
  await client.query(source);
  const gate = (
    await client.query(`SELECT
      to_regprocedure('app.read_rooming_list_scope_v3(uuid,uuid)') IS NOT NULL AS scope_read,
      has_function_privilege('smf_app','app.read_rooming_list_scope_v3(uuid,uuid)','EXECUTE') AS runtime_execute`)
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.127.0-rooming-read-scope", checksum],
    );
