...
try{
  ...
  const traveler=(await client.query(`SELECT traveler_id::text,activation_required
    ...
    candidate.actor_id,candidate.agency_id,partyId,"Viaggiatore collaudo","VC",username,email,"",
  ...
  const activated=(await client.query("SELECT * FROM app.activate_account_invitation($1,$2,$3)",
    [tokenHash,`acceptance-subject-${suffix}`,username])).rows[0];
  const reused=(await client.query("SELECT * FROM app.activate_account_invitation($1,$2,$3)",
    [tokenHash,`acceptance-subject-reuse-${suffix}`,username])).rowCount;
  const hiddenAfterUse=(await client.query("SELECT * FROM app.inspect_account_invitation($1)",[tokenHash])).rowCount===0;
  ...
  const passed=partyId&&traveler?.activation_required===true&&invitation?.email===email
    &&activated?.username===username&&reused===0&&hiddenAfterUse
  ...
  console.log(JSON.stringify({status:"passed",role,rolledBack:true,tokenSingleUse:true,
    hiddenAfterUse,reconciliation},null,2));
