...
const source = await readFile(
    new URL("../database/migrations/107_v3_activity_access_issue_grant.sql", import.meta.url),
    "utf8",
  ),
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.issue_activity_access_grant(uuid,uuid,uuid,uuid,uuid,character,character,uuid)','EXECUTE') function_ok,NOT has_table_privilege('smf_app','journey.activity_access_grants','SELECT') table_private`,
    )
  ).rows[0];
