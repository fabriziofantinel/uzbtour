...
const name = "063_v3_agency_details_management",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.update_platform_agency_details(text,uuid,jsonb)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.34.0-agency-details-management", createHash("sha256").update(source).digest("hex")],
    );
