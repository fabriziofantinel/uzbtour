...
const migrationSql = await readFile(
  new URL("../database/migrations/023_v3_traveler_experience_runtime.sql", import.meta.url),
  "utf8",
);
...
try {
  ...
  const capability = (
    await client.query(`SELECT current_user AS role_name,
  ...
  const gates = (
    ...
    EXISTS(SELECT 1 FROM pg_trigger WHERE tgname='sync_v3_contest_entry' AND tgenabled='O') AS contest_trigger`)
  ).rows[0];
  ...
  if (apply) {
    ...
    await client.query(
      ...
      VALUES($1,$2,$3) ON CONFLICT(version) DO UPDATE SET execution_ms=EXCLUDED.execution_ms,applied_at=clock_timestamp()`,
