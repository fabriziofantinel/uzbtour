...
try {
  ...
  const foreignDelete = (
    await client.query("SELECT deleted,reason FROM app.delete_legacy_demo_media($1,'photo',0)", [randomUUID()])
  ).rows[0];
  ...
  try {
    await client.query(
      `SELECT * FROM app.register_legacy_memory_upload(
    ...
    memoryScopeDenied = error?.code === "42501";
  }
    ...
    JSON.stringify(
      {
        ...
        role,
        gates,
        unknownMediaDenied: true,
        foreignMemoryDenied: memoryScopeDenied,
        foreignTicketDenied: ticketScopeDenied,
      },
      null,
      2,
    ),
