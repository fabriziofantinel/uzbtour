...
const name="035_v3_legacy_media_scope_authorization";
const model="3.8.1-legacy-media-scope-authorization";
...
try{
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-legacy-media-scope-authorization',0))");
  await client.query(source);
  const gates=(await client.query(`SELECT
    has_function_privilege('smf_app','app.resolve_legacy_demo_media_download(text,text,bigint)','EXECUTE') resolver,
    NOT has_function_privilege('smf_app','app.legacy_users_share_media_scope(text,text)','EXECUTE') helper_private,
