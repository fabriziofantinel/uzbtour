...
try {
  ...
  const payload = {
    ...
    referenceName: "Owner Acceptance",
    referenceUsername: username,
    referenceInitials: "OA",
    referenceEmail: actorRow.email,
    referencePhone: "3330000000",
    registeredCountry: "Italia",
    branding: { primaryColor: "#247A6B", logoUrl: "" },
  };
  const token = createHash("sha256").update(randomUUID()).digest("hex");
  const created = (
    await client.query(
      `SELECT * FROM app.create_platform_agency_with_owner(
    $1,$2::jsonb,$3,clock_timestamp()+interval '14 days')`,
      [actor, JSON.stringify(payload), token],
    )
  ).rows[0];
  if (!created?.agency_id || !created.activation_required)
    throw new Error("Creazione agenzia/responsabile non atomica");
  ...
  const ownerCount = Number(
    (
      await client.query(
        `SELECT count(*) total FROM iam.agency_memberships
    WHERE agency_id=$1 AND role='owner' AND status<>'revoked'`,
        [created.agency_id],
      )
    ).rows[0].total,
  );
  ...
  await client.query(
    `UPDATE iam.users users SET status='active' FROM ops.legacy_id_map map
    WHERE map.legacy_id=$1 AND map.target_id=users.id`,
    [created.legacy_user_id],
  );
  await client.query(
    `UPDATE iam.agency_memberships membership SET status='active'
    FROM ops.legacy_id_map map WHERE map.legacy_id=$1 AND map.target_id=membership.user_id
      AND membership.agency_id=$2`,
    [created.legacy_user_id, created.agency_id],
  );
  await client.query(
    ...
    [created.legacy_user_id, `acceptance-${suffix}`],
  );
  ...
  const replacement = (
    await client.query(
      `SELECT * FROM app.replace_platform_agency_owner(
  ...
  await client.query(
    ...
    FROM ops.legacy_id_map map WHERE map.legacy_id=$1 AND map.target_id=membership.user_id
      AND membership.agency_id=$2`,
    [replacement.legacy_user_id, created.agency_id],
  );
  await client.query(
    `INSERT INTO iam.user_identities(user_id,provider,subject)
    SELECT target_id,'cognito',$2 FROM ops.legacy_id_map WHERE legacy_id=$1`,
  ...
  const resolvedImpersonation = Number(
    (
      await client.query(
        `SELECT count(*) total
  ...
  const impersonationAudit = (
    await client.query(`SELECT actor_user_id::text,changes->>'targetUserId' target_user_id
  ...
  if (
    ...
    throw new Error(
      `Audit impersonazione incompleto: ${JSON.stringify({ resolvedImpersonation, impersonationAudit, actorTarget })}`,
    );
  const outsideTraveler = (
    ...
    WHERE profile.agency_id<>$1 LIMIT 1`,
      [created.agency_id],
    )
  ).rows[0];
  ...
  try {
    ...
    crossTenantDenied = error?.code === "42501";
    await client.query("ROLLBACK TO SAVEPOINT cross_tenant_impersonation");
  }
  ...
  const disabled = (await client.query("SELECT app.read_username_login_state($1) state", [replacementUsername])).rows[0]
    .state;
  const resolved = (
    await client.query("SELECT count(*)::integer total FROM app.resolve_cognito_authenticated_user($1)", [
      replacementSubject,
    ])
  ).rows[0].total;
  const suspendedImpersonation = Number(
    (
      await client.query(
        `SELECT count(*) total
    FROM app.resolve_legacy_impersonation($1,$2)`,
    ...
    JSON.stringify(
      {
        ...
        gates: {
          ...
          pendingInvite: true,
          singleOwner: true,
          activeAgencyAccess: true,
          ownerReplacement: true,
          impersonationContract: true,
          impersonationAudit: true,
          crossTenantDenied: true,
          suspendedAgencyBlocked: true,
          impersonationInvalidatedOnSuspension: true,
        },
      },
      null,
      2,
