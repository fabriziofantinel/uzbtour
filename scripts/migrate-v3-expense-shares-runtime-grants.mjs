...
try {
  ...
  const gate = (
    ...
    NOT has_table_privilege('smf_app','journey.expense_shares','TRUNCATE') truncate_denied`)
  ).rows[0];
  if (!gate || Object.values(gate).some((value) => value !== true))
    throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
  if (apply) {
    await client.query(
      `UPDATE ops.schema_migrations SET checksum_sha256=$2,applied_at=clock_timestamp()
      WHERE version=$1`,
      [model, checksum],
    );
