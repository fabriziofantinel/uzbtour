...
const name = "124_v3_photo_contest_override_visibility",
  model = "3.69.0-photo-contest-override-visibility",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT to_regprocedure('app.has_photo_contest_access_override_v3(uuid,uuid,uuid,text,uuid)') IS NOT NULL visibility_ready,has_function_privilege('smf_app','app.has_photo_contest_access_override_v3(uuid,uuid,uuid,text,uuid)','EXECUTE') runtime_ready`,
    )
  ).rows[0];
