...
try {
  ...
  await client.query(
    "SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-journey-participant-provisioning',0))",
  );
  ...
  const gates = (
    ...
    NOT EXISTS(SELECT 1 FROM public.party_memberships legacy LEFT JOIN travel.party_memberships target
      ON target.party_id=legacy.party_id AND target.traveler_id=legacy.traveler_id WHERE target.party_id IS NULL) memberships_reconciled`)
  ).rows[0];
