...
const name="039_v3_journey_participant_provisioning";
const model="3.12.0-journey-participant-provisioning";
...
try{
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-journey-participant-provisioning',0))");
  await client.query(source);
  const gates=(await client.query(`SELECT
    has_function_privilege('smf_app','app.create_journey_party(text,uuid,uuid,text,text)','EXECUTE') create_party,
    has_function_privilege('smf_app','app.provision_journey_traveler(text,uuid,uuid,text,text,text,text,date,text,text,timestamptz)','EXECUTE') provision_traveler,
    NOT has_table_privilege('smf_app','ops.legacy_id_map','SELECT') identity_map_private,
    NOT EXISTS(SELECT 1 FROM public.travel_parties legacy LEFT JOIN travel.travel_parties target ON target.id=legacy.id WHERE target.id IS NULL) parties_reconciled,
    NOT EXISTS(SELECT 1 FROM public.traveler_profiles legacy LEFT JOIN travel.traveler_profiles target ON target.id=legacy.id WHERE target.id IS NULL) travelers_reconciled,
    NOT EXISTS(SELECT 1 FROM public.party_memberships legacy LEFT JOIN travel.party_memberships target
      ON target.party_id=legacy.party_id AND target.traveler_id=legacy.traveler_id WHERE target.party_id IS NULL) memberships_reconciled`)).rows[0];
