...
const name = "064_v3_import_document_template_scope_fix",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.register_import_document_v3(text,uuid,uuid,text,text,text,text,text,bigint)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.35.0-import-document-template-scope-fix", createHash("sha256").update(source).digest("hex")],
    );
