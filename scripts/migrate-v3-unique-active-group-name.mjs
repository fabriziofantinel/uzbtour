...
const name = "206_v3_unique_active_group_name";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-unique-active-group-name',0))");
  await client.query(source);
  const gate = (
    await client.query(`SELECT
      has_function_privilege('smf_app','app.create_journey_party(text,uuid,uuid,text,text)','EXECUTE') AS runtime_granted,
      position('group name already exists' in pg_get_functiondef('app.create_journey_party(text,uuid,uuid,text,text)'::regprocedure))>0 AS duplicate_guard`)
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.132.0-unique-active-group-name", checksum],
    );
