...
const source = await readFile(
  new URL("../database/migrations/102_v3_agency_product_analytics.sql", import.meta.url),
  "utf8",
);
...
try {
  ...
  const gate = (
    await client.query(`SELECT to_regclass('ops.product_analytics_events') IS NOT NULL table_ok,
  ...
  has_table_privilege('smf_app','ops.product_analytics_events','SELECT') runtime_read_ok`)
  ).rows[0];
