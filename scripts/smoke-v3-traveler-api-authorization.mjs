...
try{
  ...
  const context=(await client.query("SELECT * FROM app.resolve_legacy_traveler_context($1,$2,$3,$4)",
    [candidate.user_id,candidate.departure_id,candidate.party_id,candidate.day_id])).rows[0];
  if(!context||String(context.agency_id)!==String(candidate.agency_id)
    ||String(context.template_version_id)!==String(candidate.template_version_id)
    ||String(context.traveler_id)!==String(candidate.traveler_id)) throw new Error("Contesto V3 non riconciliato");
  const invalid=(await client.query("SELECT * FROM app.resolve_legacy_traveler_context($1,$2,$3,$4)",
    [candidate.user_id,candidate.departure_id,candidate.party_id,randomUUID()])).rows;
