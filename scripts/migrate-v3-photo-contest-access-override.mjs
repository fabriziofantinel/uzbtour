...
const name = "123_v3_photo_contest_access_override",
  model = "3.68.0-photo-contest-access-override",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT to_regclass('journey.photo_contest_access_overrides') IS NOT NULL table_ready,pg_get_functiondef('app.upsert_photo_contest_draft_v3(text,uuid,uuid,uuid,uuid,uuid,uuid,uuid,smallint)'::regprocedure) LIKE '%photo_contest_access_overrides%' upload_ready,pg_get_functiondef('app.confirm_photo_contest_v3(text,uuid,uuid,uuid,uuid)'::regprocedure) LIKE '%photo_contest_access_overrides%' confirm_ready`,
    )
  ).rows[0];
