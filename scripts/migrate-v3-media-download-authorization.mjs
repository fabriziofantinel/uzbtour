...
const name="034_v3_media_download_authorization";
const model="3.8.0-media-download-authorization";
...
try{
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-media-download-authorization',0))");
  await client.query(source);
  const gates=(await client.query(`SELECT
    has_function_privilege('smf_app','app.resolve_legacy_memory_download(text,uuid)','EXECUTE') memory,
    has_function_privilege('smf_app','app.resolve_legacy_travel_document_download(text,uuid)','EXECUTE') document,
