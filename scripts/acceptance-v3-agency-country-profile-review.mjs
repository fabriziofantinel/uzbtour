...
try {
  ...
  if (!fixture) {
    const counts = (await client.query(`SELECT
      ...
      (SELECT count(*) FROM ops.legacy_id_map WHERE entity_type='user') mapped_users`)).rows[0];
  ...
  const listed = (await client.query("SELECT * FROM app.read_country_profiles_for_review_v3($1)",[fixture.actor_id])).rows;
  if (!listed.some((row) => String(row.agency_id)===String(fixture.agency_id) && String(row.country_id)===String(fixture.country_id))) {
    throw new Error("Il responsabile non vede il Paese della propria agenzia");
  }
  const updated = (await client.query(
    "SELECT app.review_country_profile_v3($1,$2,$3,true) updated",
    [fixture.actor_id,fixture.agency_id,fixture.country_id],
  )).rows[0]?.updated;
  ...
  `,[fixture.agency_id,fixture.country_id])).rows[0];
  ...
  if (superadmin) {
    ...
    try {
      await client.query("SELECT app.review_country_profile_v3($1,$2,$3,true)",[
        superadmin.legacy_id,fixture.agency_id,fixture.country_id,
      ]);
  ...
  open = false;
  console.log(JSON.stringify({status:"passed_with_rollback",ownerCanReview:true,superadminDenied:true,agencyScoped:true}));
} finally {
