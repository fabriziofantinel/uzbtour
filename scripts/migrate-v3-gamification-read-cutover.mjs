...
const name="031_v3_gamification_read_cutover";
const model="3.5.0-gamification-read-cutover";
...
try{
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-gamification-read-cutover',0))");
  await client.query(source);
  const gates=(await client.query(`SELECT
    (SELECT count(*) FROM pg_trigger WHERE tgname='zz_enrich_v3_activity_attempt' AND tgenabled='O')=1 enrichment_trigger,
    NOT has_table_privilege('smf_app','ops.legacy_generated_content_map','SELECT') content_map_private,
    has_table_privilege('smf_app','content.activity_items','SELECT') activity_items,
    has_table_privilege('smf_app','journey.activity_attempts','SELECT') attempts,
    NOT EXISTS(
      SELECT 1 FROM journey.activity_attempts attempt
      CROSS JOIN LATERAL jsonb_each(attempt.answers) answer
      WHERE NOT (answer.value ? '__resultId' AND answer.value ? '__score' AND answer.value ? '__status')
    ) enriched_answers`)).rows[0];
