...
const name = "126_v3_photo_contest_full_ai_reason",
  model = "3.71.0-photo-contest-full-ai-reason",
...
try {
  ...
  const definition =
    (
      await client.query(
        "SELECT pg_get_functiondef('app.complete_photo_contest_evaluation_v3(uuid,uuid[],text,jsonb)'::regprocedure) definition",
      )
    ).rows[0]?.definition ?? "";
  const gate = { full_reason_ready: !definition.includes("left(COALESCE(value ->> 'reason'::text, ''::text), 500)") };
  if (!gate.full_reason_ready) throw new Error(`Gate incompleto: ${JSON.stringify(gate)}`);
  if (apply) {
    await client.query(
