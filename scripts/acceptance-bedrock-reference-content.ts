...
function assertPhotoValidation(
  items: Array<{
    photoValidation?: { target: string; requiredFeatures: string[]; rejectIf: string[]; minimumConfidence: number };
  }>,
  label: string,
) {
  if (
    items.some(
      (item) =>
        !item.photoValidation ||
        !item.photoValidation.target ||
        item.photoValidation.requiredFeatures.length === 0 ||
        item.photoValidation.rejectIf.length === 0 ||
        item.photoValidation.minimumConfidence < 0.65,
    )
  ) {
...
function assertGames(
  games: Array<{ type: string; pairs?: unknown[]; options?: unknown[]; correctIndex?: number }>,
  label: string,
) {
  ...
  if (oddOneOut?.options?.length !== 4 || oddOneOut.correctIndex == null)
    throw new Error(`Trova l'intruso non valido: ${label}`);
}
