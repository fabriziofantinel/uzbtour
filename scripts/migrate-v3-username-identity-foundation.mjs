...
const name="055_v3_username_identity_foundation",model="3.27.0-username-identity-foundation";
...
try{
  ...
  const contract=(await client.query(`SELECT
    ...
    has_function_privilege('smf_app','app.provision_platform_agency_agent(text,uuid,text,text,text,text,text,text,text,timestamptz)','EXECUTE') username_agent_provisioning`)).rows[0];
  if(!contract?.username_required||!contract.email_not_unique||!contract.cognito_resolver||!contract.username_provisioning||!contract.username_agent_provisioning)
  ...
  await client.query(`INSERT INTO iam.users(username,display_name,email,status) VALUES
  ...
  const count=Number((await client.query("SELECT count(*) total FROM iam.users WHERE normalized_email=$1",[shared])).rows[0].total);
  ...
  console.log(JSON.stringify({status:apply?"applied":"dry_run_passed",role:role.role_name,executionMs,
    gates:{usernameRequired:true,emailShared:true,cognitoResolver:true,usernameProvisioning:true,usernameAgentProvisioning:true},
    migration:{name,model,checksum}},null,2));
