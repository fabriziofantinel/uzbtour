...
const name = "065_v3_trip_deletion_target_authorization",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.resolve_trip_deletion_target_v3(text,uuid)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.36.0-trip-deletion-target-authorization", createHash("sha256").update(source).digest("hex")],
    );
