...
const name = "036_v3_media_mutation_authorization";
const model = "3.9.0-media-mutation-authorization";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-media-mutation-authorization',0))");
  await client.query(source);
  const gates = (
    await client.query(`SELECT
    has_function_privilege('smf_app','app.register_legacy_memory_upload(text,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,bigint)','EXECUTE') memory,
    has_function_privilege('smf_app','app.register_legacy_ticket_upload(text,uuid,uuid,uuid,uuid,text,text,text,text,text,bigint)','EXECUTE') ticket,
    has_function_privilege('smf_app','app.delete_legacy_demo_media(text,text,bigint)','EXECUTE') legacy_delete,
