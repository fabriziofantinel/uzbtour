...
const url=process.env.DATABASE_MIGRATION_URL??process.env.DATABASE_URL_UNPOOLED??process.env.DATABASE_URL;if(!url)throw new Error("Connessione diretta Neon owner non configurata");
const client=new Client(url);let open=false;
try{
  await client.connect();await client.query("BEGIN");open=true;
  const actor=(await client.query(`SELECT map.legacy_id,map.target_id FROM ops.legacy_id_map map JOIN iam.users u ON u.id=map.target_id WHERE map.source_system='public-v2' AND map.entity_type='user' AND u.platform_role='superadmin' AND u.status='active' ORDER BY u.created_at LIMIT 1`)).rows[0];
  if(!actor)throw new Error("Superadmin di collaudo non disponibile");
  const agencyId=crypto.randomUUID(),templateId=crypto.randomUUID(),versionId=crypto.randomUUID(),refJobId=crypto.randomUUID(),candidateUserId=crypto.randomUUID();
  ...
  await client.query(`INSERT INTO iam.users(id,username,display_name,email,platform_role,status)
    VALUES($1,$2,'Acceptance Agent','acceptance@example.invalid','user','active')`,
    [candidateUserId,`acceptance.${candidateUserId.slice(0,8)}`]);
  await client.query(`INSERT INTO ops.legacy_id_map(source_system,entity_type,legacy_id,target_id,agency_id)
    VALUES('public-v2','user',$1,$2,$3)`,[`acceptance:${candidateUserId}`,candidateUserId,agencyId]);
  await client.query(`INSERT INTO iam.agency_memberships(agency_id,user_id,role,status)
    VALUES($1,$2,'admin','active')`,[agencyId,candidateUserId]);
  await client.query(`INSERT INTO iam.impersonation_sessions(actor_user_id,target_user_id,token_hash,reason,expires_at)
  ...
  const finalized=Number((await client.query(`SELECT app.finalize_agency_deletion_identities_v3($1) deleted`,[deletion.job_id])).rows[0].deleted);
  const checks=(await client.query(`SELECT NOT EXISTS(SELECT 1 FROM iam.agencies WHERE id=$1) agency_removed,
    ...
    [agencyId,deletion.job_id,candidateUserId])).rows[0];
  if(!checks?.agency_removed||!checks?.job_completed||!checks?.identity_removed||!checks?.session_removed||finalized!==1)
    throw new Error(`Postcondizioni non rispettate: ${JSON.stringify({checks,finalized})}`);
  await client.query("ROLLBACK");open=false;
  console.log(JSON.stringify({status:"passed",tripCrud:true,referenceDirectWrite:true,agencyDeletionBr019:true,
    identityFinalization:true,finalPhase:state.phase,transaction:"rolled_back"},null,2));
