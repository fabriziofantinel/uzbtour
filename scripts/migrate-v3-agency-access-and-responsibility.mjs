...
const name="056_v3_agency_access_and_responsibility",model="3.28.0-agency-access-and-responsibility";
...
try{
  ...
  const contract=(await client.query(`SELECT
    ...
    EXISTS(SELECT 1 FROM pg_indexes WHERE schemaname='iam'
      AND indexname='agency_memberships_single_owner_uidx') owner_unique`)).rows[0];
  if(!contract||Object.values(contract).some(value=>!value))throw new Error("Contratto accesso agenzia incompleto");
  ...
  console.log(JSON.stringify({status:apply?"applied":"dry_run_passed",role:role.role_name,executionMs,
    gates:contract,migration:{name,model,checksum}},null,2));
