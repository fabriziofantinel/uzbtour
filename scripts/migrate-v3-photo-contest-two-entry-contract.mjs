...
const name="110_v3_photo_contest_two_entry_contract",model="3.36.0-photo-contest-two-entry-contract";
const source=await readFile(new URL(`../database/migrations/${name}.sql`,import.meta.url),"utf8"),checksum=createHash("sha256").update(source).digest("hex");
...
const gate=(await client.query(`SELECT
  ...
  pg_get_constraintdef((SELECT oid FROM pg_constraint WHERE conrelid='journey.photo_contest_entries'::regclass AND conname='photo_contest_entries_participant_slot_check')) LIKE '%participant_slot <= 2%' entry_constraint`)).rows[0];
if(!gate||Object.values(gate).some(v=>v!==true))throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
if(apply){await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256)VALUES($1,$2)ON CONFLICT(version)DO UPDATE SET applied_at=clock_timestamp()`,[model,checksum]);await client.query("COMMIT");}else await client.query("ROLLBACK");open=false;
console.log(JSON.stringify({status:apply?"applied":"dry_run_passed",gate,migration:{name,model,checksum}}));}catch(error){if(open)await client.query("ROLLBACK").catch(()=>{});throw error;}finally{await client.end().catch(()=>{});}
