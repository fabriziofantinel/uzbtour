...
try{
  ...
  const unknownInspection=(await client.query(
    "SELECT * FROM app.inspect_account_invitation($1)",[unknownHash],
  )).rowCount===0;
  const unknownActivation=(await client.query(
    "SELECT * FROM app.activate_account_invitation($1,$2,$3)",
    [unknownHash,randomUUID(),`${randomUUID()}@invalid.example`],
  )).rowCount===0;
  ...
  console.log(JSON.stringify({status:"passed",role,gates,
    unknownInvitationHidden:unknownInspection,unknownActivationDenied:unknownActivation},null,2));
