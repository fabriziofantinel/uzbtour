...
const source = await readFile(
  new URL("../database/migrations/100_v3_impersonation_audit.sql", import.meta.url),
  "utf8",
);
...
try {
  ...
  const gate = (
    await client.query(`SELECT EXISTS(SELECT 1 FROM pg_trigger WHERE tgname='impersonation_session_audit_v3' AND tgenabled<>'D') trigger_ok,
    has_function_privilege(current_user,'app.audit_impersonation_session_v3()','EXECUTE') owner_execute_ok`)
  ).rows[0];
