...
const name = "097_v3_photo_evidence_attempt_limit",
  model = "3.64.0-photo-evidence-attempt-limit",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.submit_photo_evidence_v3(text,uuid,uuid,uuid,uuid,uuid,uuid,uuid)','EXECUTE') submit_ok`,
    )
  ).rows[0];
  if (!gate?.submit_ok) throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
  if (apply) {
    await client.query(
