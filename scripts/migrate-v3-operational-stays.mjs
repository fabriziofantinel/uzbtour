...
const name = "045_v3_operational_stays",
  model = "3.17.0-operational-stays",
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-operational-stays',0))");
  await client.query(source);
  const gates = (
    await client.query(
      `SELECT to_regclass('travel.template_accommodation_stays') IS NOT NULL template_stays,to_regclass('travel.departure_accommodation_stays') IS NOT NULL departure_stays,has_table_privilege('smf_app','travel.departure_accommodation_stays','SELECT') runtime_read`,
    )
  ).rows[0];
