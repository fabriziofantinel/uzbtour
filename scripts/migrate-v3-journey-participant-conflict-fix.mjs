...
try {
  ...
  await client.query(
    "SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-journey-participant-conflict-fix',0))",
  );
  ...
  const gates = (
    ...
    pg_get_functiondef('app.provision_journey_traveler(text,uuid,uuid,text,text,text,text,date,text,text,timestamptz)'::regprocedure)
      NOT LIKE '%ON CONFLICT(party_id,traveler_id)%' conflict_target_fixed`)
  ).rows[0];
