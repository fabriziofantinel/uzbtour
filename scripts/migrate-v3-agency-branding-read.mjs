...
try {
  ...
  const gate = (
    await client.query("SELECT has_function_privilege('smf_app','app.read_agency_branding_v3(text)','EXECUTE') ok")
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.47.0-agency-branding-read", createHash("sha256").update(source).digest("hex")],
    );
