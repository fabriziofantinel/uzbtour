...
const name = "113_v3_web_push_identity_contract",
  model = "3.39.0-web-push-identity-contract",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.upsert_own_web_push_subscription_v3(text,text,text,text,text,timestamptz)','EXECUTE') runtime_execute,pg_get_functiondef('app.upsert_own_web_push_subscription_v3(text,text,text,text,text,timestamptz)'::regprocedure) LIKE '%iam.user_identities%' cognito_identity,pg_get_functiondef('app.upsert_own_web_push_subscription_v3(text,text,text,text,text,timestamptz)'::regprocedure) LIKE '%DISTINCT ON(profile.agency_id)%' multi_agency`,
    )
  ).rows[0];
