...
const name = "096_v3_minor_photo_consent",
  model = "3.63.0-minor-photo-consent",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.read_journey_management(text,uuid)','EXECUTE') read_ok,has_function_privilege('smf_app','app.set_minor_image_consent_v3(text,uuid,uuid,uuid,uuid,text,text)','EXECUTE') write_ok`,
    )
  ).rows[0];
