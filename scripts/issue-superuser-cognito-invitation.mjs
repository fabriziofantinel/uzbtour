...
try {
  ...
  const linked = await client.query(`SELECT 1 FROM iam.user_identities WHERE user_id=$1 AND provider='cognito'`, [user.id]);
  if (linked.rowCount) throw new Error("Il superuser e' gia' collegato a Cognito");
  await client.query(`UPDATE iam.invitations SET used_at=clock_timestamp() WHERE invited_user_id=$1 AND used_at IS NULL`, [user.id]);
  await client.query(`UPDATE public.user_invitations SET used_at=clock_timestamp() WHERE user_id=$1 AND used_at IS NULL`, [user.legacy_id]);
