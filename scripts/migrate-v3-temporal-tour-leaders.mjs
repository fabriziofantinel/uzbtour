...
const name = "140_v3_temporal_tour_leader_assignments";
...
try {
  ...
  const gate = (
    ...
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='travel' AND table_name='departure_staff_assignments' AND column_name='valid_until' AND is_nullable='NO') temporal,
    has_function_privilege('smf_app','app.revoke_tour_leader_v3(uuid,uuid,uuid,text)','EXECUTE') revocation`)
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.85.0-temporal-tour-leaders", checksum],
    );
