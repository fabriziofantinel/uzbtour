...
const merged = mergeReconciliationIssues(draft, [
  {
    ...
    severity: "blocking",
    fieldPath: "endDate",
    message: "Duplicato dal revisore AI",
    sourceText: "12-10 ottobre",
    resolved: false,
  },
  {
    ...
    message: "La data nel riepilogo differisce.",
    sourceText: "11 ottobre",
    resolved: false,
  },
]);
    ...
    days: [
      {
        ...draft.days[0],
        dayNumber: 1,
        accommodation: { ...draft.days[0].accommodation, name: "Hotel Cumbres Lasterra" },
      },
    ],
  },
...
assert.equal(
  travelProgrammeDraftSchema.parse(correctedHotel.value).days[0]?.accommodation.name,
...
console.log(
  JSON.stringify({
    status: "passed",
    detected: [...codes].sort(),
    mergedIssues: merged.reconciliationIssues.length,
    sourceDateCorrections: 3,
  }),
);
