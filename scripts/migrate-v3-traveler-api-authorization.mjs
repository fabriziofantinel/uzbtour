...
const name = "033_v3_traveler_api_authorization";
const model = "3.7.0-traveler-api-authorization";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-traveler-api-authorization',0))");
  await client.query(source);
  const gates = (
    await client.query(`SELECT
    has_function_privilege('smf_app','app.resolve_legacy_traveler_context(text,uuid,uuid,uuid)','EXECUTE') context,
    has_function_privilege('smf_app','app.resolve_legacy_traveler_scope(text,uuid,uuid,uuid)','EXECUTE') scope,
