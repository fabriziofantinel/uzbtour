...
try {
  ...
  WHERE membership.status='active' ORDER BY day.service_date DESC LIMIT 1`)
  ).rows[0];
  if (!fixture) throw new Error("Fixture contest non disponibile");
  await client.query("UPDATE travel.departure_days SET service_date=current_date WHERE id=$1", [
  ...
  for (let index = 0; index < 2; index++) {
    const row = (
      await client.query(
        `SELECT * FROM app.upsert_photo_contest_draft_v3(
  $1,$2,$3,$4,$5,$6,$7,$8,NULL)`,
    ...
    if (row.status !== "draft" || Number(row.participant_slot) !== index + 1)
      throw new Error("Bozza contest non valida");
    entries.push(row.id);
  }
  await client.query("SAVEPOINT third_photo");
  let thirdDenied = false;
  try {
    await client.query(
      `SELECT * FROM app.upsert_photo_contest_draft_v3(
  $1,$2,$3,$4,$5,$6,$7,$8,NULL)`,
    ...
    thirdDenied = error?.code === "23505";
    await client.query("ROLLBACK TO SAVEPOINT third_photo");
  }
  if (!thirdDenied) throw new Error("La terza foto non e stata rifiutata");
  const replaced = (
    await client.query(`SELECT * FROM app.upsert_photo_contest_draft_v3($1,$2,$3,$4,$5,$6,$7,$8,1::smallint)`, [
      fixture.actor_legacy_id,
      fixture.agency_id,
      fixture.departure_id,
      fixture.party_id,
  ...
  if (Number(replaced.participant_slot) !== 1) throw new Error("Sostituzione slot non riuscita");
  const confirmed = (
    await client.query("SELECT * FROM app.confirm_photo_contest_v3($1,$2,$3,$4,$5)", [
      fixture.actor_legacy_id,
      fixture.agency_id,
      fixture.departure_id,
      fixture.party_id,
  ...
  const evaluations = confirmed.entry_ids.map((id, index) => ({
    entryId: id,
    total: index === 0 ? 72 : 88,
    theme: index === 0 ? 35 : 45,
    quality: index === 0 ? 37 : 43,
    reason: `acceptance-${index + 1}`,
  }));
  const best = (
    await client.query(
      "SELECT app.complete_photo_contest_evaluation_v3($1,$2::uuid[],'acceptance-model',$3::jsonb) best",
      [fixture.agency_id, confirmed.entry_ids, JSON.stringify(evaluations)],
    )
  ).rows[0]?.best;
  if (String(best) !== String(confirmed.entry_ids[1])) throw new Error("Selezione della foto migliore non coerente");
  await client.query("UPDATE travel.departure_days SET service_date=current_date-2 WHERE id=$1", [
    fixture.departure_day_id,
  ]);
  const closed = Number((await client.query("SELECT app.close_due_photo_contests_v3() closed")).rows[0]?.closed ?? 0);
  const ranked = (
    await client.query(
      `SELECT count(*)::int total,count(*) FILTER(WHERE status='ranked' AND is_winner)::int winners
  FROM journey.photo_contest_entries WHERE id=ANY($1::uuid[])`,
      [confirmed.entry_ids],
    )
  ).rows[0];
  if (closed < 1 || ranked.total !== 2 || ranked.winners !== 1)
    throw new Error(`Chiusura contest non coerente: ${JSON.stringify({ closed, ranked })}`);
