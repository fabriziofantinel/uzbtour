...
const url=process.env.DATABASE_MIGRATION_URL??process.env.DATABASE_URL_UNPOOLED;if(!url)throw new Error("Connessione Neon owner non configurata");
...
try{await client.connect();
 ...
 WHERE membership.status='active' LIMIT 1`)).rows[0];if(!fixture)throw new Error("Fixture engagement non disponibile");
 const ticketFixture=(await client.query(`SELECT item.agency_id,item.departure_id,
  ...
  WHERE item.item_type IN('flight','train') LIMIT 1`)).rows[0];
 await client.query("BEGIN");open=true;await client.query("GRANT smf_app TO current_user");await client.query("SET LOCAL ROLE smf_app");
 await client.query("SELECT set_config('app.agency_id',$1,true)",[fixture.agency_id]);
 const mediaId=randomUUID(),memoryId=randomUUID(),feedbackOp=randomUUID();
 ...
 const memory=(await client.query(`SELECT memory_id::text,created_at::text FROM app.register_legacy_memory_upload($1,$2,$3,$4,$5,$6,'r2','acceptance',$7,'ricordo.jpg','image/jpeg',1024)`,
  [fixture.legacy_id,fixture.departure_id,fixture.party_id,fixture.template_day_id,mediaId,memoryId,objectKey])).rows[0];
 let ticketRegistered=null;if(ticketFixture){const ticketMedia=randomUUID(),ticketDocument=randomUUID();
  ...
  ticketRegistered=(await client.query(`SELECT document_id::text,title FROM app.register_legacy_ticket_upload($1,$2,$3,$4,$5,'r2','acceptance',$6,'biglietto.pdf','application/pdf',2048)`,
   [ticketFixture.actor_legacy_id,ticketFixture.departure_id,ticketFixture.legacy_item_id,ticketMedia,ticketDocument,ticketKey])).rows[0];}
 const feedback=(await client.query(`WITH actor AS(SELECT traveler.id FROM travel.traveler_profiles traveler
  ...
  RETURNING id::text,rating`,[fixture.agency_id,fixture.departure_id,fixture.party_id,fixture.legacy_id,
    fixture.departure_day_id,fixture.departure_item_id,feedbackOp])).rows[0];
 const checks=(await client.query(`SELECT
   ...
   [fixture.agency_id,fixture.party_id,memoryId,mediaId,fixture.departure_id,fixture.departure_item_id])).rows[0];
 const list=(await client.query(`SELECT item.title,avg(feedback.rating)::numeric(4,2) average,count(*) responses
   ...
   [fixture.agency_id,fixture.departure_id,fixture.departure_item_id])).rows[0];
 if(!memory?.memory_id||!feedback?.id||Number(checks.memories)!==1||Number(checks.media)!==1||Number(checks.feedback)<1||!list?.title)
 ...
 await client.query("ROLLBACK");open=false;console.log(JSON.stringify({status:"passed_with_rollback",memoryRegistered:true,
  ticketRegistered:ticketFixture?true:"no_fixture",partyScoped:true,feedbackSaved:true,feedbackAnalytics:true,
  itemTitle:list.title,rating:Number(list.average),responses:Number(list.responses)}));
