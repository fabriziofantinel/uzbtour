...
const name = "066_v3_trip_deletion_assets_scope_fix",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.read_trip_deletion_assets_v3(text,uuid,uuid)','EXECUTE') ok`,
    )
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      ["3.37.0-trip-deletion-assets-scope-fix", createHash("sha256").update(source).digest("hex")],
    );
