...
try {
  ...
  const fixture = (
    await client.query(`SELECT media.agency_id,media.departure_id,media.party_id,map.legacy_id
   ...
   WHERE media.purpose='memory' ORDER BY media.created_at DESC LIMIT 1`)
  ).rows[0];
  ...
  const rows = (
    await client.query(
      `SELECT * FROM app.enqueue_platform_job_v3($1,$2,'photo-evidence.validate','database',$3::jsonb,$4,clock_timestamp())`,
      [fixture.legacy_id, fixture.agency_id, JSON.stringify(payload), `scope-probe-${randomUUID()}`],
    )
  ).rows;
  const gate = { executable: rows.length === 1 };
  if (!gate.executable) throw new Error(`Gate incompleti: ${JSON.stringify(gate)}`);
  await client.query("DELETE FROM ops.platform_jobs WHERE id=$1", [rows[0].id]);
  if (apply) {
    await client.query(
      `INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms) VALUES('3.72.0-traveler-job-scope',$1,0)
