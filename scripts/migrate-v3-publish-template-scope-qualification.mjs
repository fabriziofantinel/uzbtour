...
const name = "071_v3_publish_template_scope_qualification",
...
try {
  ...
  const gate = {
    ...
    qualifiedDelete: definition.includes("template_country.template_id=v_template"),
    qualifiedCount: definition.includes("template_country_count.template_id=v_template"),
    noUnqualified:
      !definition.includes("WHERE template_id=v_template") && !definition.includes("AND template_id=v_template"),
  };
  ...
  if (apply) {
    await client.query(
      ...
      ["3.42.0-publish-template-scope-qualification", createHash("sha256").update(source).digest("hex")],
    );
