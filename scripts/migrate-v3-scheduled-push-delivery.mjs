...
const name = "094_v3_scheduled_push_delivery",
  model = "3.61.0-scheduled-push-delivery",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT to_regclass('ops.push_delivery_runs') IS NOT NULL table_ok,has_function_privilege('smf_app','app.claim_due_push_deliveries_v3()','EXECUTE') claim_ok,has_function_privilege('smf_app','app.complete_push_delivery_v3(uuid,boolean,integer,integer,text)','EXECUTE') complete_ok`,
    )
  ).rows[0];
