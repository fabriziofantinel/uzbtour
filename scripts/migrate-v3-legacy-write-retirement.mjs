...
const name = "050_v3_legacy_write_retirement";
const model = "3.22.0-legacy-write-retirement";
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-legacy-write-retirement',0))");
  await client.query(source);
  const gates = (
    await client.query(`
    SELECT
      NOT has_table_privilege('smf_app','public.party_expenses','INSERT') expense_retired,
      NOT has_table_privilege('smf_app','public.party_activity_results','UPDATE') activity_retired,
      NOT has_table_privilege('smf_app','public.generated_content','DELETE') content_retired,
      NOT EXISTS(
        SELECT 1 FROM pg_trigger trigger_row
        JOIN pg_class relation ON relation.oid=trigger_row.tgrelid
        JOIN pg_namespace namespace ON namespace.oid=relation.relnamespace
        WHERE namespace.nspname='public' AND trigger_row.tgname LIKE 'sync_v3_%'
          AND NOT trigger_row.tgisinternal) shadow_triggers_retired
  `)
  ).rows[0];
  ...
  if (apply) {
    await client.query(
      ...
      applied_at=clock_timestamp(),execution_ms=EXCLUDED.execution_ms`,
