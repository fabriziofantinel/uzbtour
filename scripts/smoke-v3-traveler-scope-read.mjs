...
try{
  ...
  const legacy=(await owner.query(`SELECT departure.id::text departure_id,party.id::text party_id
    ...
    WHERE profile.user_id=$1 AND departure.status NOT IN('cancelled','archived')`,[legacyUser.user_id])).rows;
  ...
  const allowed=(await client.query("SELECT app.resolve_legacy_traveler_scope($1,$2,$3,NULL) agency_id",
    [legacyUser.user_id,first.departure_id,first.party_id])).rows[0]?.agency_id;
  if(String(allowed)!==String(first.agency_id)) throw new Error("Risoluzione scope valida fallita");
  const denied=(await client.query("SELECT app.resolve_legacy_traveler_scope($1,$2,$3,NULL) agency_id",
    [legacyUser.user_id,randomUUID(),first.party_id])).rows[0]?.agency_id;
