...
const name = "042_v3_superadmin_read_cutover",
  model = "3.14.0-superadmin-read",
...
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(hashtextextended('smf-travel:v3-superadmin-read',0))");
  await client.query(source);
  const gates = (
    await client.query(
      `SELECT has_function_privilege('smf_app','app.read_superadmin_summary(text)','EXECUTE') summary,has_function_privilege('smf_app','app.read_superadmin_agency_registry(text)','EXECUTE') registry,has_function_privilege('smf_app','app.read_superadmin_impersonation_users(text)','EXECUTE') users,NOT has_table_privilege('smf_app','ops.legacy_id_map','SELECT') identity_map_private`,
    )
  ).rows[0];
