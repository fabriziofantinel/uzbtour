...
try{
  ...
  const gate=(await client.query(`SELECT
    ...
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='ops' AND table_name='travel_documents' AND column_name='departure_day_id') day_ready`)).rows[0];
  ...
  if(apply){await client.query(`INSERT INTO ops.schema_migrations(version,checksum_sha256,execution_ms)
    ...
    ["3.50.0-departure-day-documents",createHash("sha256").update(source).digest("hex")]);await client.query("COMMIT");}
