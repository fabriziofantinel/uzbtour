...
try {
  ...
  const journey = (await client.query("SELECT * FROM app.list_legacy_user_journeys($1) LIMIT 1", [legacyUserId]))
    .rows[0];
  if (!journey) throw new Error("Viaggio del viaggiatore non trovato");
  const context = (
    await client.query(
      `SELECT membership.traveler_id::text,day.id::text departure_day_id,
  ...
  await client.query(
    `INSERT INTO journey.expenses(id,agency_id,departure_id,party_id,departure_day_id,label,
    ...
    [
      expenseId,
      journey.agency_id,
      journey.departure_id,
      journey.party_id,
      context.departure_day_id,
      context.traveler_id,
      randomUUID(),
    ],
  );
  await client.query(
    `INSERT INTO journey.cash_movements(id,agency_id,departure_id,party_id,departure_day_id,kind,
  ...
  if (
    visibility.current_expense !== 1 ||
    visibility.other_expense !== 0 ||
    visibility.current_cash !== 2 ||
    visibility.other_cash !== 0
  )
    ...
    JSON.stringify(
      {
        ...
        partyId: journey.party_id,
        otherPartyId: context.other_party_id,
        operations: { expense: true, withdrawal: true, exchange: true },
        visibility,
      },
      null,
      2,
