...
const name = "090_v3_async_ocr_resume",
...
try {
  ...
  const gate = (
    await client.query(
      `SELECT to_regprocedure('app.mark_import_ocr_pending_v3(uuid,text)') IS NOT NULL AS mark_ready,to_regprocedure('app.resume_import_ocr_v3(uuid,text)') IS NOT NULL AS resume_ready`,
    )
  ).rows[0];
  if (!gate?.mark_ready || !gate?.resume_ready) throw new Error("Contratto OCR incompleto");
  if (apply) {
    await client.query(
      ...
      ["3.60.0-async-ocr-resume", createHash("sha256").update(source).digest("hex")],
    );
