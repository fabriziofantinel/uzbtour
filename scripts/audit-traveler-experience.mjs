...
try {
  const candidates = (
    await client.query(
      `SELECT map.legacy_id AS legacy_user_id
    ...
    WHERE map.legacy_id=$1 OR lower(account.display_name) LIKE lower('%'||$1||'%')`,
      [userSelector],
    )
  ).rows;
  if (candidates.length !== 1)
    throw new Error(`Il selettore utente deve individuare un solo profilo (trovati: ${candidates.length})`);
  ...
  for (const journey of journeys) {
    ...
    const [
      ...
      client.query(
        ...
        (SELECT count(*) FROM travel.template_phrasebook_entries WHERE agency_id=$1 AND template_version_id=$3) phrases`,
        [journey.agency_id, journey.departure_id, journey.template_version_id],
      ),
      client.query(
        `SELECT COALESCE(day.day_number,0) AS day_number,activity.activity_type,count(item.id)::int items,
        ...
        GROUP BY day.day_number,activity.activity_type ORDER BY day.day_number,activity.activity_type`,
        [journey.agency_id, journey.template_version_id],
      ),
      client.query(
      ...
      client.query(
        ...
        (SELECT count(*) FROM journey.cash_movements WHERE agency_id=$1 AND departure_id=$2 AND party_id=$3)::int cash`,
        [journey.agency_id, journey.departure_id, journey.party_id],
      ),
      client.query(
        `SELECT count(*)::int total,count(*) FILTER(WHERE id=$3)::int current
        FROM travel.travel_parties WHERE agency_id=$1 AND departure_id=$2`,
        [journey.agency_id, journey.departure_id, journey.party_id],
      ),
      client.query(
        `WITH entities AS(
        ...
        ORDER BY entity.entity_type,content.content_type,content.status`,
        [journey.agency_id, journey.departure_id, journey.template_version_id],
      ),
      client.query(
        `SELECT count(*)::int total,
      ...
      client.query(
        `SELECT activity.availability_rule,count(*)::int activities,
        ...
        GROUP BY activity.availability_rule`,
        [journey.agency_id, journey.departure_id, journey.template_version_id],
      ),
      client.query(
        `SELECT count(*)::int candidates FROM travel.traveler_profiles traveler
      ...
      client.query(
        `SELECT traveler.id::text traveler_id,traveler.user_id::text profile_user_id,
    ...
    reports.push({
      journey: {
        ...
        departureId: journey.departure_id,
        title: journey.title,
        country: journey.destination_country,
        party: journey.party_name,
        agency: journey.agency_name,
        branding: journey.agency_branding,
      },
      ...
      referenceContent: referenceContent.rows,
      accessGrants: accessGrants.rows[0],
      quizEligibility: quizEligibility.rows,
      runtimeCandidates: runtimeCandidates.rows[0],
      identityDiagnostic: identityDiagnostic.rows,
    });
  }
