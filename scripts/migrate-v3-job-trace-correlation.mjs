...
const name = "132_v3_job_trace_correlation",
  model = "3.77.0-job-trace-correlation",
...
try {
  ...
  const gate = (await client.query(`SELECT to_regclass('ops.platform_jobs_trace_idx') IS NOT NULL trace_index`))
    .rows[0];
  if (!gate?.trace_index) throw new Error("Gate trace incompleto");
  if (apply) {
    await client.query(
