...
try {
  ...
  const partyId = (
    await client.query("SELECT app.create_journey_party($1,$2,$3,$4,$5)::text id", [
      ...
      `LIFE-${suffix.slice(0, 10)}`,
      "Gruppo lifecycle rollback",
    ])
  ).rows[0].id;
  const people = [];
  for (const [index, role] of ["organizer", "member", "member"].entries()) {
    ...
    const activated = (
      await client.query("SELECT * FROM app.activate_account_invitation($1,$2,$3)", [
        token,
        `lifecycle-subject-${suffix}-${index}`,
        username,
      ])
    ).rows[0];
    if (!traveler?.traveler_id || activated?.username !== username)
      throw new Error("Attivazione fixture lifecycle fallita");
    people.push({ username, travelerId: traveler.traveler_id });
  }
  if (ownerFallback) await client.query("RESET ROLE");
  const actors = (
    await client.query(
      `SELECT map.legacy_id id,users.username,users.status user_status,
    ...
    WHERE users.username=ANY($1::text[])`,
      [people.map((person) => person.username), partyId],
    )
  ).rows;
  ...
  const consentId = (
    await client.query("SELECT app.set_minor_image_consent_v3($1,$2,$3,$4,$5,'granted','fixture rollback')::text id", [
      candidate.actor_id,
      candidate.agency_id,
      candidate.departure_id,
      partyId,
  ...
  const registered = (
    await client.query(
      `SELECT document_id::text FROM app.register_departure_day_document(
    $1,$2,$3,$4,$5,$6,'r2','smf-test',$7,'voucher.pdf','application/pdf',128,'Voucher collaudo')`,
      [candidate.actor_id, candidate.departure_id, candidate.day_id, partyId, mediaId, documentId, objectKey],
    )
  ).rows[0]?.document_id;
  const download = (
    await client.query("SELECT * FROM app.resolve_legacy_travel_document_download($1,$2)", [leaderActor, documentId])
  ).rows[0];
  const foreignDownload = (
    await client.query("SELECT * FROM app.resolve_legacy_travel_document_download($1,$2)", [randomUUID(), documentId])
  ).rowCount;
  const archived = (
    await client.query("SELECT app.archive_day_document_v3($1,$2,$3,$4) archived", [
      candidate.actor_id,
      candidate.agency_id,
      candidate.departure_id,
      documentId,
  ...
  const competition = (
    await client.query("SELECT app.update_own_trip_competition_v3($1,$2,$3,true) updated", [
      leaderActor,
      candidate.departure_id,
      partyId,
    ])
  ...
  const transferred = (
    await client.query("SELECT app.transfer_own_group_leadership_v3($1,$2,$3,$4) transferred", [
      leaderActor,
      candidate.departure_id,
      partyId,
      people[1].travelerId,
  ...
  const removedMember = (
    await client.query("SELECT app.remove_journey_traveler_v3($1,$2,$3,$4,$5) removed", [
      candidate.actor_id,
      candidate.agency_id,
      candidate.departure_id,
      partyId,
  ...
  const deleted = (
    await client.query("SELECT app.delete_empty_journey_party_v3($1,$2,$3,$4) deleted", [
      candidate.actor_id,
      candidate.agency_id,
      candidate.departure_id,
      partyId,
  ...
  const auditCount = Number(
    (
      await client.query(
        `SELECT count(*) count FROM ops.audit_events
    WHERE agency_id=$1 AND entity_id=ANY($2::text[]) AND action IN
      ('trip_competition_settings_updated','leader_transferred_by_traveler','removed_from_journey','archived','minor_image_consent_updated')`,
        [candidate.agency_id, [partyId, ...people.map((person) => person.travelerId)]],
      )
    ).rows[0].count,
  );
  if (
    ...
    throw new Error(
      `Lifecycle incompleto: ${JSON.stringify({
        consentId,
        registered,
