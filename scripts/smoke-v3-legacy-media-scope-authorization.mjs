...
try{
  ...
  const candidate=(await client.query(`SELECT 'photo' kind,id,uploaded_by_id owner_id FROM public.trip_photos
  ...
  const ownerAuthorized=candidate
    ? (await client.query("SELECT 1 FROM app.resolve_legacy_demo_media_download($1,$2,$3)",
        [candidate.owner_id,candidate.kind,candidate.id])).rowCount===1
    : null;
  if(ownerAuthorized===false) throw new Error("Il proprietario non puo leggere il proprio media");
  const foreignDenied=(await client.query("SELECT 1 FROM app.resolve_legacy_demo_media_download($1,$2,$3)",
    [randomUUID(),candidate?.kind??"photo",candidate?.id??0])).rowCount===0;
