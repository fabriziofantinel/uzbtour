...
try{await client.connect();await client.query("BEGIN");open=true;
 const fixture=(await client.query(`SELECT map.legacy_id,party.agency_id,party.departure_id,party.id party_id,day.id day_id
  ...
  WHERE membership.status='active' AND NOT EXISTS(SELECT 1 FROM iam.agency_memberships agency_member
    WHERE agency_member.user_id=profile.user_id AND agency_member.status='active') LIMIT 1`)).rows[0];if(!fixture)throw new Error("Fixture viaggiatore attivo non disponibile");
 ...
 const sessionEvent=(await client.query(`SELECT app.record_product_analytics_event_v3($1,$2,$3,$4,'traveler_session',$5,$6,'{"smoke":true}'::jsonb)::text id`,
  [fixture.legacy_id,fixture.departure_id,fixture.party_id,fixture.day_id,session,crypto.randomUUID()])).rows[0].id;
 const documentEvent=(await client.query(`SELECT app.record_product_analytics_event_v3($1,$2,$3,$4,'document_download',$5,$6,$7::jsonb)::text id`,
  [fixture.legacy_id,fixture.departure_id,fixture.party_id,fixture.day_id,session,crypto.randomUUID(),JSON.stringify({smoke:true,documentId:crypto.randomUUID()})])).rows[0].id;
 const chatId=(await client.query("SELECT app.send_operational_message_v3($1,$2,$3,'Richiesta assistenza smoke',$4)::text id",
  [fixture.legacy_id,fixture.departure_id,fixture.party_id,crypto.randomUUID()])).rows[0].id;
 ...
 const measures=(await client.query(`SELECT
   ...
   FROM ops.product_analytics_events WHERE agency_id=$1 AND departure_id=$2`,[fixture.agency_id,fixture.departure_id])).rows[0];
 const assistance=Number((await client.query(`SELECT count(*) count FROM journey.operational_messages
   WHERE agency_id=$1 AND departure_id=$2 AND party_id=$3 AND sender_role='traveler'`,
  [fixture.agency_id,fixture.departure_id,fixture.party_id])).rows[0].count);
 const chat=(await client.query("SELECT sender_role,sender_traveler_id::text traveler_id FROM journey.operational_messages WHERE id=$1",[chatId])).rows[0];
 if(first!==second||event?.event_name!=="programme_view"||event?.smoke!=="true"||!sessionEvent||!documentEvent||!chatId
   ||Number(measures.active_users)<1||Number(measures.document_users)<1||assistance<1)
   throw new Error(`Gate analytics fallito: ${JSON.stringify({first,second,sessionEvent,documentEvent,chatId,chat,measures,assistance})}`);
 await client.query("ROLLBACK");open=false;console.log(JSON.stringify({status:"passed_with_rollback",idempotent:first===second,
  rlsRead:true,aggregateReadable:Number(aggregate.events)>=1,distinctActors:Number(aggregate.actors)>=1,
  activeUsers:Number(measures.active_users),documentUsers:Number(measures.document_users),assistanceRequests:assistance}));
