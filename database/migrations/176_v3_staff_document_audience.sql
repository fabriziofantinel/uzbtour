-- Documents can be delivered to the assigned Accompagnatori or Guide of a departure.

ALTER TABLE ops.travel_documents ADD COLUMN IF NOT EXISTS staff_role TEXT;
ALTER TABLE ops.travel_documents DROP CONSTRAINT IF EXISTS travel_documents_staff_role_ck;
ALTER TABLE ops.travel_documents ADD CONSTRAINT travel_documents_staff_role_ck
  CHECK (staff_role IS NULL OR staff_role IN ('accompagnatore','guida'));
...
BEGIN
 ...
 RETURN QUERY INSERT INTO ops.travel_documents(id,agency_id,departure_id,departure_day_id,party_id,traveler_id,staff_role,media_asset_id,document_type,title,description,status)
 VALUES(p_document_id,v_agency,p_departure_id,p_day_id,p_party_id,p_traveler_id,v_staff_role,p_media_id,'other',p_original_name,v_description,'ready') RETURNING ops.travel_documents.id,ops.travel_documents.title,ops.travel_documents.description,ops.travel_documents.created_at;
END $$;
...
SET search_path=pg_catalog,iam,travel,ops SET row_security=off AS $$
 ...
 OR EXISTS(SELECT 1 FROM travel.departure_staff_assignments assignment WHERE assignment.agency_id=document.agency_id AND assignment.departure_id=document.departure_id AND assignment.user_id=p_actor_user_id AND assignment.status='active' AND document.staff_role IS NOT NULL AND assignment.role IN(document.staff_role,CASE WHEN document.staff_role='accompagnatore' THEN 'tour_leader' ELSE document.staff_role END))) LIMIT 1;
$$;
...
INSERT INTO public.platform_schema_migrations(version) VALUES('176_v3_staff_document_audience') ON CONFLICT(version) DO NOTHING;
