-- Allinea l'editor del programma pubblicato alla revisione senza creare nuove versioni.
-- Le rimozioni restano tracciate come cancellazioni operative per preservare l'audit.
...
BEGIN
  FOR v_constraint IN
    ...
    WHERE constraint_schema='travel'
      AND check_clause LIKE '%scheduled_start_at%'
      AND check_clause LIKE '%item_type%'
  LOOP
    EXECUTE format('ALTER TABLE travel.departure_itinerary_items DROP CONSTRAINT %I',v_constraint);
  END LOOP;
END $$;

ALTER TABLE travel.departure_itinerary_items
  ADD CONSTRAINT departure_items_timed_type_check CHECK(
    (scheduled_start_at IS NULL AND scheduled_end_at IS NULL)
    OR item_type IN('transport','flight','train','meal','meeting')
  );
...
DECLARE
  v_actor UUID;v_agency UUID;v_version UUID;v_service_date DATE;v_timezone TEXT;v_row JSONB;
...
BEGIN
  ...
  FOR v_row IN SELECT value FROM jsonb_array_elements(p_stays) LOOP
    ...
    IF NOT FOUND THEN
      INSERT INTO travel.departure_accommodation_stays(
        ...
        v_stay_id,v_agency,p_departure_id,v_version,p_day_id,v_row->>'name',
  ...
  VALUES(v_agency,v_actor,'departure_day',p_day_id::text,'updated',jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION app.update_departure_programme_day_v3(TEXT,UUID,UUID,TEXT,TEXT,TEXT,TEXT,JSONB,JSONB) TO smf_app;

INSERT INTO public.platform_schema_migrations(version)
VALUES('078_v3_operational_programme_full_edit') ON CONFLICT(version) DO NOTHING;
