-- Supporta più pernottamenti per giornata e orari per pasti/incontri in pubblicazione.
...
BEGIN
  ...
  JOIN LATERAL(SELECT id FROM travel.trip_template_versions
    WHERE agency_id=import_job.agency_id AND template_id=import_job.template_id AND status='draft'
...
INSERT INTO public.platform_schema_migrations(version) VALUES('069_v3_multi_stay_review') ON CONFLICT(version) DO NOTHING;
