-- Guides can only use their direct agency conversation. Standard trip, group and traveler chats remain unavailable.
...
BEGIN
  ...
  IF v_staff_role='guida' THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='guide standard chat not authorized';END IF;
...
SET search_path=pg_catalog,app,iam,travel,journey,ops SET row_security=off AS $$
DECLARE v_agency UUID;v_sender_traveler UUID;v_sender_party UUID;v_name TEXT;v_role TEXT;v_staff BOOLEAN;v_staff_role TEXT;v_id UUID;
BEGIN
  SELECT agency_id INTO v_agency FROM travel.departures WHERE id=p_departure;
...
REVOKE ALL ON FUNCTION app.list_operational_messages_scoped_v3(UUID,UUID,TEXT,UUID,UUID,INTEGER),
  app.send_operational_message_scoped_v3(UUID,UUID,TEXT,UUID,UUID,TEXT,UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.list_operational_messages_scoped_v3(UUID,UUID,TEXT,UUID,UUID,INTEGER),
  app.send_operational_message_scoped_v3(UUID,UUID,TEXT,UUID,UUID,TEXT,UUID) TO smf_app;

INSERT INTO public.platform_schema_migrations(version)
VALUES('193_v3_guide_chat_boundary') ON CONFLICT(version) DO NOTHING;
