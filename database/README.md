...
1. original application tables from `scripts/migrate.mjs`;
2. legacy multitenant foundation `001-011` from `scripts/migrate-platform.mjs`;
3. hardening migrations `012-015`;
4. normalized v3 target baseline from `database/schema-v3-review.sql`;
5. incremental migrations `019-128` in numeric order;
6. structural, RLS, constraint, index and tenant-isolation validation.
